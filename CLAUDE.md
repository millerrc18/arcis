...
- **Test count must not drop** — CI enforces a minimum of 5400 tests (Sprint 5 close baseline as of 2026-05-13). Floor lineage: 3038 (pre-audit) → 3159 (Track 1) → 3238 (Cohort 1) → 3380 (Cohort 2 + 3A) → 3646 (Track 1.5 + Round 10, with `--ignore=test_sprint_F_engine.py`) → 3651 (PR-690 I5: dropped `--ignore` after engine fixture regen, +5 tests) → 3671 (Sprint 1.A.0: Wikipedia scraper tests +9 + PIT loader tests +11) → 3682 (Sprint 1.A.1: T10 migration regression-locks +11) → 4798 (Sprint 4 close: cockpit followups + notifications observability) → 5400 (Sprint 5 close: cutover stabilization + notification subsystem + LLM packet enrichment + AST scanners + Wave C7b plan-gates + T16 fold-ins). The pg-tests.yml CI floor (5050 → 5100 post-Sprint-5) is intentionally conservative to absorb the chronic-failure class (worktree env-drift, hardcoded test:test fixtures, env-pollution). Bump this number in CLAUDE.md whenever the sweep grows past the previous baseline.
