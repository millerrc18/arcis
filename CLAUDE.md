...
- **Test count must not drop** — CI enforces a minimum of 3646 tests (post-Track-1.5 Integrator sweep baseline as of 2026-04-25; +266 net from 3380 Cohort-2+3A floor across Track 1.5 Pass 2 + Round 10 work: B1/B2.A/B2.B/B2.C/B3/B4/B5/B6/B7/B8/B9/B10 instrumentation rounds + 8.A-8.F dashboard fixes + 5-KPI hero strip + Round-10 Fix-A/B/C/D zero-failures cleanup). Floor lineage: 3038 (pre-audit) → 3159 (Track 1) → 3238 (Cohort 1) → 3380 (Cohort 2 + 3A) → 3646 (Track 1.5 + Round 10). **Zero failures, zero errors at this baseline** (full sweep: `python -m pytest tests/ -q --timeout=60 --ignore=tests/platform/byte_identity/test_sprint_F_engine.py` → 3646 passed, 11 skipped in 999.31s on 2026-04-25). Bump this number in CLAUDE.md whenever the sweep grows past the previous baseline.
