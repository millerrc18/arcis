...
- **Every PR updates `CHANGELOG.md` under `[Unreleased]`** — versioning policy in `docs/versioning-policy.md`. Releases bump `src/version.py` + create a git tag. Drift between `src/version.py`, `CHANGELOG.md` headers, and tags is the failure mode this rule prevents.
- **Test count must not drop** — CI enforces a minimum of 5467 tests (#111 arcis:periodic-discipline baseline as of 2026-05-26). Floor lineage: 3038 (pre-audit) → 3159 (Track 1) → 3238 (Cohort 1) → 3380 (Cohort 2 + 3A) → 3646 (Track 1.5 + Round 10, with `--ignore=test_sprint_F_engine.py`) → 3651 (PR-690 I5: dropped `--ignore` after engine fixture regen, +5 tests) → 3671 (Sprint 1.A.0: Wikipedia scraper tests +9 + PIT loader tests +11) → 3682 (Sprint 1.A.1: T10 migration regression-locks +11) → 4798 (Sprint 4 close: cockpit followups + notifications observability) → 5300 (Sprint 5 close: cutover stabilization + notification subsystem + LLM packet enrichment + AST scanners + Wave C7b plan-gates + T16 fold-ins; 5469 collected) → 5388 (#94 dual-GPU re-cutover: deleted 49 vram_manager tests (T14, 5 files), added 137 new behavioral tests across the watchdog/training-control/startup-guard/liveness/gpu-pin/handler-registration suites; net −49+137=+88; 6064 collected) → 5467 (#111 arcis:periodic-discipline: +79 new tests collected — 28 test functions × parametrize expansion. Across tests/skills/test_periodic_discipline.py (20 collected), tests/skills/test_periodic_discipline_boundary.py (54 collected, parametrized over 13 tools), tests/tools/test_workflow_parity_scanner.py (5 collected); new tests/skills/ directory). The pg-tests.yml CI floor (5050 → 5100 post-Sprint-5 → 5188 post-#94 → 5267 post-#111: +79 collected) is intentionally conservative to absorb the chronic-failure class (worktree env-drift, hardcoded test:test fixtures, env-pollution). Bump this number in CLAUDE.md whenever the sweep grows past the previous baseline.
- **Mock all external APIs in tests** — no network calls from pytest (Alpaca, Finnhub, yfinance, FRED, Ollama)
- **Schema registry is the single source of truth** — all tables are defined in `src/schema/registry.py` (authoritative count via `len(TABLES)`: `python -c "from src.schema.registry import TABLES; print(len(TABLES))"`). See "Database Schema Rules" below
...
Two repo-root structure rules added in Phase 5 PR-A (DD-39): `test_no_underscore_scratch_at_repo_root` (forbids `_*.py` REPL scratch at the repo root, excluding `__init__.py` / `__main__.py`) and `test_no_sqlite_at_repo_root` (forbids `*.sqlite` / `*.sqlite3` / `*.sqlite-journal` / `*.db` at the repo root — the runtime DB lives at `C:/arcis/data/ai_research_desk.sqlite3` per `CLAUDE.md:26`). Each is paired with a `tmp_path`-anchored sentinel proving non-vacuousness.

**Boundary-touch standards (T0a):** PRs that touch an external seam (DB / LLM / HTTP / NSSM / ripgrep / broker) must add boundary-touch tests per [`docs/standards/boundary-touch-tests.md`](docs/standards/boundary-touch-tests.md) — drive both sides of the seam with real artifacts (no mocks at the seam) and prove the test would fail if the implementation were deleted. The PR template (`.github/PULL_REQUEST_TEMPLATE.md`) carries the 6-item self-check.
...
- `src/scheduler/holidays.py` — uses `pandas_market_calendars` (NEW dep, see below) for NYSE calendar + half-day handling. **`subtract_trading_days(anchor: date, n: int) -> date`** is the canonical helper for trading-day arithmetic in fetch anchors and lookback windows. When writing code that needs a fetch anchor or lookback window (e.g. "200 trading days before this date"), USE `subtract_trading_days()` from this module. Do NOT use `timedelta(days=365)` or other calendar-day approximations — 291–293 calendar days = exactly 200 trading days depending on holiday density, and the approximation drifts. Background: the #888 / #106 incident traced corpus generator + backtester silent data gaps to this exact drift (PRs #922, #923).
