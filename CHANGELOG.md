...
- **feat(frontend,test): T22 — E1.C ESLint custom rule + pytest fixture for queryFn guardrail.** Sprint 3 cockpit-coherence T22 deliverable. Creates `frontend/eslint-rules/no-bare-queryfn-with-args.js` — ESLint rule that visits `useQuery()` calls and reports an error when `queryFn` is a bare `MemberExpression` (e.g. `api.getStatus`) instead of an arrow function, using message `useQuery queryFn is a bare reference; use () => api.foo() arrow form to prevent QueryFunctionContext leakage as first arg`. Arrow functions and function expressions are never flagged. Registers the rule in `frontend/eslint.config.js` as `local/no-bare-queryfn-with-args: error` via an inline local plugin. Adds `lint:queryfn` script in `frontend/package.json` that runs `eslint --no-config-lookup -c eslint-rules/eslint.queryfn.config.js src/` (dedicated config lints only the bare-queryFn rule, not all pre-existing lint errors). Creates `tests/test_eslint_queryfn_guardrail.py` with two tests: `test_lint_passes_current_frontend` asserts `lint:queryfn` exits 0 on the post-T17-T21 frontend (all bare refs wrapped); `test_lint_fails_on_bare_queryfn` injects a synthetic JSX file with `queryFn: api.foo` and asserts the lint exits non-zero (rule is live). Both tests pass. CI-enforcement: any future PR reintroducing `queryFn: api.method` bare member access will fail `lint:queryfn` and the pytest guardrail.

- **fix(frontend): T18 — E1.A2 TanStack v5 bare-queryFn sweep (Dashboard, ModelPerformance, StressTest, Training).** Sprint 3 cockpit-coherence T18 deliverable. Wraps 11 bare `queryFn: api.method` references in arrow-function form (`queryFn: () => api.method()`) across 4 files. `Dashboard.jsx` (7 sites): `api.getStatus` (L222), `api.getTrainingStatus` (L225), `api.getHaltStatus` (L227), `api.getLatestAudit` (L228), `api.getConfig` (L230), `api.getBuildScore` (L232), `api.getSystemIndex` (L241). `ModelPerformance.jsx` (1 site): `api.getModelPerformance` (L35). `StressTest.jsx` (1 site): `api.getStressTestResults` (L37). `Training.jsx` (3 sites): `api.getTrainingStatus` (L20), `api.getTrainingVersions` (L21), `api.getDataCollectionStats` (L22). Already-correct arrow-form calls (e.g., `api.getOpenTrades(deskFilter)`, `api.getClosedTrades(30, deskFilter)`, `fetchApi('/kpis')`) were not changed. Sibling-search confirmed no additional bare-ref sites remain in any of the 4 files. 12 new tests in `frontend/src/pages/T18.test.jsx` assert `queryFn !== api.method` (bare-ref check) and `typeof queryFn === 'function'` for each wrapped call site across all 4 components.

- **fix(frontend): T19 — E1.A3 TanStack v5 bare-queryFn sweep (Docs, Validation, TradeHistory).** Sprint 3 cockpit-coherence T19 deliverable. Wraps 3 bare `queryFn: api.method` references in arrow-function form (`queryFn: () => api.method()`) so TanStack Query v5 does not receive the `QueryFunctionContext` object as the first argument. Primary bug fix: `TradeHistory.jsx:238` (`api.getSharpeAttribution`) — `getSharpeAttribution(desk)` takes an optional `desk` parameter, so the bare ref caused TanStack v5 to pass the context object as `desk`, serialized as `?desk=%5Bobject+Object%5D` in the URL. Additional wraps: `Docs.jsx:204` (`api.getDocsList`), `Validation.jsx:92` (`api.getValidation`). `TradeHistory.jsx:232` (`() => api.getClosedTrades(180)`) was already correctly arrow-wrapped. Sibling-search: no remaining bare `queryFn: api.` sites in any of the 3 files. 3 new tests in `frontend/src/pages/T19.test.jsx` asserting `queryFn !== api.method` (bare-ref check) and `typeof queryFn === 'function'` for each wrapped call site.

- **fix(frontend): T20 — E1.B TanStack v5 bare-queryFn sweep (Attribution, Settings, Health, Monitoring).** Sprint 3 cockpit-coherence T20 deliverable. Wraps 8 bare `queryFn: api.method` references in arrow-function form (`queryFn: () => api.method()`) across 4 files previously edited by prior batch tasks. `Attribution.jsx:10` (`api.getAttributionStats`). `Settings.jsx:187` (`api.getConfig`), `Settings.jsx:188` (`api.getStatus`), `Settings.jsx:190` (`api.getSettings`). `Health.jsx:54` (`api.getHSHS`), `Health.jsx:59` (`api.getBuildScore`), `Health.jsx:63` (`api.getTrainingHistory`), `Health.jsx:68` (`api.getIBStatus`). `Monitoring.jsx` already had correct arrow form for both queries (`getMonitoringHistory(24)` and `fetchApi('/monitoring/snapshot')`) — no changes needed. Sibling-search: no additional bare-ref sites found in any of the 4 files. 11 new tests across 4 test files assert `queryFn !== api.method` (bare-ref check) and `typeof queryFn === 'function'` for each wrapped call site. New test file: `Attribution.test.jsx` (2 tests). Tests appended to existing files: `Settings.test.jsx` (+3 tests), `Health.test.jsx` (+4 tests), `Monitoring.test.jsx` (+2 tests).

- **fix(frontend): T21 — E1.B2 TanStack v5 bare-queryFn sweep (LiveLedger, Council, Simulation, ShadowLedger).** Sprint 3 cockpit-coherence T21 deliverable. Closes audit findings 03-C1 + 05-C3. Wraps 6 bare `queryFn: api.method` references in arrow-function form (`queryFn: () => api.method()`) so TanStack Query v5 does not receive the `QueryFunctionContext` object as the first argument. The 3 ShadowLedger sites (L476 `api.getOpenTrades`, L478 `api.getAccount`, L481 `api.getLiveTrades`) were the primary `desk=[object Object]` regression sources: `getOpenTrades(desk)` and `getAccount(desk)` take an optional `desk` parameter, so TanStack passing the context object as the first arg serialized as `?desk=%5Bobject+Object%5D` in the URL. Additional wraps: `LiveLedger.jsx:164` (`api.getLiveSummary`), `LiveLedger.jsx:167` (`api.getLiveTrades`), `Council.jsx:288` (`api.getCouncilLatest`), `Simulation.jsx:51` (`api.getSimulationResults`). Sibling-search confirmed no further bare-queryFn sites remain in any of the 4 files; `DiagnosticKickoffButtons.jsx` read-only check confirmed it has no `useQuery` calls (only `useMutation`/`useQueryClient` — no scope expansion needed). 7 new tests in `frontend/src/pages/T21.test.jsx` assert `queryFn !== api.method` (bare-ref check) and `typeof queryFn === 'function'` for each wrapped call site.

- **feat(frontend): T15 — F2.B Settings IB toggle migration to visually-disabled with whyDisabled tooltip.** Sprint 3 cockpit-coherence T15 deliverable per spec §7 F2.B decision matrix. `live_trading.ib.shadow_mode` and `live_trading.ib.paper_routing` toggles in `Settings.jsx` are now visually disabled because they only take effect when the local IB Gateway is reachable (cliOnly semantics). Implementation: added `whyDisabled: 'Effect requires local IB Gateway connection'` to both SETTING_META entries; `SettingInput` toggle branch detects `meta.whyDisabled` and renders a `data-ib-key`-attributed wrapper with a disabled button (no `onClick`, `disabled` attribute set, `cursor-not-allowed opacity-40`) and the `whyDisabled` text rendered inline below the description — always in the DOM, no hover required. Click handler is a true no-op: the `onUpdate` callback is never referenced in the disabled branch, so `api.updateSettings` is never called on IB toggle interactions. Non-IB toggles (`shadow_trading.enabled`, `llm.enabled`) are fully unchanged — BC preserved. 7 new tests in `frontend/src/pages/Settings.test.jsx` covering: shadow_mode disabled button present, shadow_mode whyDisabled text visible, paper_routing disabled button present, paper_routing whyDisabled text visible (both rows show it, `getAllByText` asserts `>=2`), click on shadow_mode does NOT fire mutation, click on paper_routing does NOT fire mutation, non-IB toggle click still fires mutation (BC lock). Visual-verify: worktree has no dev server; disclosed as limitation — changes are pure DOM attribute additions, no CSS class overrides, no layout changes to non-IB rows.

- **fix(frontend): T17 — E1.A TanStack v5 bare-queryFn sweep (Layout, RevenueProjection, IBShadow, Notes).** Sprint 3 cockpit-coherence T17 deliverable. Wraps 4 bare `queryFn: api.method` references in arrow-function form (`queryFn: () => api.method()`) so TanStack Query v5 does not receive the `QueryFunctionContext` object as the first argument to the API call, which previously caused `?desk=[object Object]` URL corruption. Wraps: `Layout.jsx:130` (`api.getStatus`), `RevenueProjection.jsx:87` (`api.getProjectionsLive`), `IBShadow.jsx:58` (`api.getIBShadowSummary`), `Notes.jsx:54` (`api.fetchNotes`). The existing arrow-form calls in these files (`api.getKpis()` in Layout, `api.getIBShadowLog(50)` in IBShadow) were already correct and are unchanged. Sibling-search: no additional bare-ref sites found in any of the 4 files. 7 new tests across 4 test files assert `queryFn !== api.method` (bare-ref check) and `typeof queryFn === 'function'` for each wrapped call site.

- **feat(frontend): T13 — C2 migrate 4 widgets to LoadingState.** Sprint 3 cockpit-coherence T13 deliverable. Replaces ad-hoc loading/error patterns in 4 widgets with the shared `<LoadingState>` component (T6 deliverable). **BrokerExceptionsPanel.jsx**: imports `LoadingState`; passes `isLoading`, `isError`, `error`, `retry` (from `refetch`) with `compact={true}` for inline error display; `isEmpty` derived as `!isLoading && !isError && rows.length === 0`; `emptyMessage="No exceptions yet"`. Removed old inline `if (isLoading) return <div>Loading...</div>` and `if (isError || !data) return <div>Failed to load</div>` branches. **DBSchema.jsx**: adds `isLoading`, `isError`, `error`, `refetch` to useQuery destructure; wraps the ReactFlow card in `<LoadingState>` — shows spinner on load, error card on failure, renders graph on success. Removed old silent `catch(() => ({}))` fallback that was masking errors. **Health.jsx**: replaces `import LoadingSpinner` with `import LoadingState`; the `if (hshsLoading && buildLoading) return <LoadingSpinner />` early return replaced with `<LoadingState isLoading={true} ...>`; the `!hasHshs` branch replaces bare `<div>Collecting HSHS data...</div>` with `<LoadingState isEmpty={true} emptyMessage="Collecting HSHS data..." />` (also surfaces `hshsError` / `hshsErrorObj` / `refetchHshs` from useQuery so HSHS error state renders via LoadingState instead of silently showing empty). **Monitoring.jsx**: replaces `import LoadingSpinner` with `import LoadingState`; the `if (isLoading) return <LoadingSpinner />` branch extended to `if (isLoading || isError)` returning `<LoadingState isLoading={isLoading} isError={isError} error={error} retry={refetch} ...>` — **CRUCIAL: closes E7 presentation bug** where a 500/503 from `/api/monitoring/history` would show an infinite spinner instead of an error card (T3 wired the 200+empty+note fallback at the backend; T13 ensures the frontend can also handle genuine errors gracefully). All 4 pass `isError` from useQuery per C2 spec requirement. 11 new tests across 4 test files (3 new test files, 4 tests added to BrokerExceptionsPanel.test.jsx, 2 added to Health.test.jsx, 3 new in DBSchema.test.jsx, 3 new in Monitoring.test.jsx).

- **test(api): T16 — B3 CI dashboard reconciliation test (SQLite-only, cohort-aware).** Sprint 3 cockpit-coherence T16 deliverable. Creates `tests/test_dashboard_reconciliation.py` with 8 parametrized+individual tests that regression-lock the `_meta` envelope contract across 5 main dashboard endpoints (`/api/cto-report`, `/api/shadow/metrics`, `/api/status`, `/api/attribution/stats`, `/api/stress-test/results`). Cohort-aware reconciliation pattern: `test_closed_count_reconciles` checks `cto._meta.trade_summary.cohort == shadow._meta.cohort` BEFORE asserting `n == n`; cohort mismatch skips (by design) rather than fails. `test_open_position_reconcile` verifies `/api/status.open_positions == /api/live/summary.open_positions` under a controlled fixture. `test_invalid_cohort_id_rejected` validates the 8-cohort closed taxonomy enforced by `meta_entry()`. All tests use mock runtime; no network calls, no Postgres — SQLite-only per spec §5 B3. Postgres validation deferred to Sprint 4 `#SP4-render-pg-reconcile`.

- **feat(frontend): T14 — F2 migrate 4 pages to ActionButton + profit_factor null-handling.** Sprint 3 cockpit-coherence. `LiveLedger.jsx`: Reconcile button replaced with `<ActionButton cliOnly={true} cliCommand='python -m src.main reconcile-live' whyDisabled='Requires local broker auth'>` — tooltip shows CLI command with copy button (T7 ActionButton spec). `DiagnosticKickoffButtons.jsx`: 3 buttons (nightly audit, packet writer, snapshot) replaced with `<ActionButton cliOnly={false}>` preserving existing `onClick` mutations; `pending` prop wired to `isActive || mutation.isPending`. `Simulation.jsx`: 2 run buttons (empty-state + header) each replaced with `ActionButton` — mutually exclusive render paths, so exactly 1 renders at a time (DEDUPE satisfied); `profit_factor null` now renders `'N/A (no losses)'` (T4 dependency: engine.py now emits `None` for no-loss scenarios). `Council.jsx`: Run Council Now + Ask Council both replaced with `<ActionButton cliOnly={false}>` preserving `onClick`. `StressTest.jsx`: no profit_factor column so no change needed — renders without crash when field is null. 17 tests in `frontend/src/pages/T14.test.jsx`. Removed `Tooltip` and raw disabled-button patterns from `LiveLedger.jsx`.

- **fix(frontend): T12 fix — A4 meta consumption spec-drift resolution + missing tests.** QA follow-up to PR #997. (1) **Spec-field investigation**: spec asked for 4 fields (`win_rate`, `n_trades`, `rf_adjusted_excess_sharpe`, `total_pnl_dollars`). After investigation: `win_rate` wired (badge on WinRateCard); `rf_adjusted_excess_sharpe` wired (badge on RfAdjustedCard); `n_trades` is equivalent-via-meta-n — the WinRateCard meta badge's `.n` field equals `n_trades` because the backend `meta_entry("kpi.canonical", n_trades)` call at `kpis.py:115` sets `n=n_trades`, so it is surfaced on the strip. `total_pnl_dollars` has no primary value card in the 5-card strip — no card shows dollar P&L as its headline value; a TODO comment tagged `#SP3-T12-pnl-card` is added in `KPIStrip.jsx` for Sprint 4 follow-up when a P&L dollar card is added. (2) **Scope expansion acknowledged**: `KPIStrip.jsx` is added to `files_in_scope` (was not in original T12 scope) because Dashboard.jsx only renders `<KPIStrip kpis={...}/>` — per-card meta wiring must live inside KPIStrip, not Dashboard. (3) **Missing tests added**: `frontend/src/pages/Strategy.test.jsx` (NEW, 2 tests) — `renders strategy-meta-badge when data._meta is present`, `renders no strategy-meta-badge when data._meta is absent`. `KPIStrip.test.jsx` extended with `KPIStrip _meta envelope wiring` describe block (4 tests): rf-Adj badge present with _meta.rf_adjusted_excess_sharpe, win-rate badge present with _meta.win_rate, two badges when both present, zero badges when _meta absent. Total frontend tests: 121 → 127 (+6). All 127 pass.

- **feat(frontend): T12 — A4 Dashboard/TradeHistory/Strategy meta consumption.** Sprint 3 cockpit-coherence T12 deliverable. Wires `_meta` cohort badges through 3 pages. (1) `KPIStrip.jsx`: `RfAdjustedCard` and `WinRateCard` now accept and forward a `meta` prop to `KPICard` (T5 added the `meta` prop to `KPICard` but deferred sub-card wiring to T12). `KPIStrip` main function passes `safeKpis._meta?.rf_adjusted_excess_sharpe` and `safeKpis._meta?.win_rate` respectively — these are populated by the T8/T9 `_meta` envelope on `/api/kpis`. (2) `TradeHistory.jsx`: inline cohort badge (`data-testid="attribution-meta-badge"`) rendered below the Excess Sharpe number in the Primary Metric card when `attribution._meta?.sharpe_ratio` is present. Uses same `n=N · last-segment` format as KPICard. Imports `Tooltip` component. (3) `Strategy.jsx`: inline cohort badge (`data-testid="strategy-meta-badge"`) rendered below the KPI grid when `data?._meta` is present — uses T9-wired `_meta` from `/api/strategy-detail`. Imports `TooltipComponent`. (4) `TradeHistory.test.jsx` (NEW): 2 tests — badge renders from `attribution._meta.sharpe_ratio` when present; no badge when `_meta` absent. All existing 112 tests continue to pass; 2 new tests added (total 114).

- **fix(api): T9 follow-up — correct §2.3 cohort mapping for `/api/shadow/metrics`.** QA reviewer found `trades.live_only` was mapped to `desk='all'` (SQL `1=1`, no filter) — the opposite of its definition (`trades.all_closed AND source='live'`). `_desk_clause()` filters by the `desk` column, not the `source` column, so no current desk value produces a `source='live'` SQL predicate. Hardcoded `cohort_id = 'trades.all_closed'` for all desk values until a true `source='live'` filter is wired (Sprint 4 follow-up `#SP4-shadow-metrics-live-cohort`). Replaced tautological test `test_shadow_metrics_desk_live_emits_live_only` with `test_shadow_metrics_all_desks_emit_all_closed` (iterates `[None, 'swing', 'live', 'all', 'research_a']` — all must emit `trades.all_closed`). Sprint 4 cleanup must update this test when `source='live'` filter is wired.

- **feat(api): T9 — A1.B backend `_meta` envelope on remaining 7 endpoints.** Sprint 3 cockpit-coherence T9 deliverable. Wires `meta_entry()` from `src/api/cohort_meta.py` (T8 helper) into 7 remaining API endpoints: `/api/shadow/metrics` (`trades.all_closed` for all desk values per §2.3 correction above), `/api/attribution/stats` (`attribution.pairs`, n=`paired_n`), `/api/strategy-detail/{strategy_type}` (`trades.strategy`), `/api/model-performance` (`trades.model`, in `training.py:549`), `/api/build-score` (`none`), `/api/health/hshs` (per-section: `overall=none`, `performance=trades.all_closed`), `/api/stress-test/results` (`stress.scenario`, n=scenario count), `/api/simulation/results` (`none`). All changes are additive — no existing numeric keys modified (BC). Imports added to `trades.py` and `training.py`. 9 tests in `tests/test_cto_report.py::TestT9MetaEnvelopes`. Sibling-search finding: `/api/traffic-light/current`, `/api/health/score` in analytics.py and `/api/monitoring/history`, `/api/monitoring/snapshot` also lack `_meta` — not in T9 scope, documented as follow-up.

- **feat(frontend): T6 — C1 shared LoadingState component (with retry cooldown).** New `frontend/src/components/LoadingState.jsx` wrapping existing `LoadingSpinner` and `EmptyState`. Implements 3-state machine (loading → data | empty | error) with `retryDisabledFor={ms}` cooldown to prevent retry spam. API: `isLoading`, `isError`, `error`, `retry`, `retryDisabledFor` (default 0), `isEmpty`, `loadingMessage`, `emptyMessage`, `compact`, `children`. Render logic: spinner path shows `LoadingSpinner` + `loadingMessage`; error path shows error card with `error.message` + retry button (disabled for `retryDisabledFor` ms after click via `useEffect` timer); empty path delegates to `EmptyState`; data path renders children. `compact=true` renders inline without card wrapper. 5 tests in `frontend/src/components/LoadingState.test.jsx` covering all paths including retry-cooldown via `vi.useFakeTimers()`. Sprint 3 cockpit-coherence Batch 0 T6 deliverable.
