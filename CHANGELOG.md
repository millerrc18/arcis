...
<<<<<<< HEAD
- **#858 Section 8 options-flow source audit** — `docs/research/section-8-options-source-audit.md`. **MAJOR FINDING**: pre-reg addendum 1 §A2.2 placeholder treatment is WRONG. There IS a live producer (`src/data_collection/options_metrics.py` writes to `options_metrics` table; schema is PIT-capable with `collected_at` + `collected_date` indexes). The runtime loader `src/features/engine_helpers.py::_load_options_metrics` is PIT-broken (`WHERE collected_at = (SELECT MAX(collected_at) ...)`) AND has a field-name mismatch (prompt expects `iv_skew_25d`, loader returns `iv_skew`) AND drops 2 of the 6 schema columns from its SELECT (`iv_percentile`, `atm_iv_30d`). Net: 3 of 6 prompt fields are populated runtime-only (PIT violation in any historical-decision context); 3 of 6 always render 'n/a'. Phase 4 corpus generation as-merged silently leaks future options data into historical decision points. PM-rescue from terminated `design-codebase-analyst` agent (78s silent termination — same pattern as Phase 2 audit dispatch). PM recommends Option A (fix the loader to accept `as_of` + complete the SELECT + fix the field name) to land before Stage 1; alternatives B/C/D documented. **Pre-reg addendum-2 required before Phase 6 (Stage 1 backtest)** because the §A2.2 "no live producer" statement is now known to be wrong.
- **#96.2 LLM-scoring corpus generator** (Sprint 1.C Phase 4 sub-task 2 of 4) — `src/evaluation/corpus_generator.py` + `scripts/generate_llm_corpus.py`. `generate_corpus(...)` enumerates walk-forward decision points, calls `enrich_features(..., as_of=as_of)` for PIT-clean enrichment, builds the prompt via `_build_feature_prompt` (frozen at v0.32.0 per §A1.3), calls `enhance_packet_with_llm`, captures `llm_conviction` + `llm_conviction_parse_failed`, and streams `CorpusEntry` rows to `entries.jsonl` + `manifest.json`. `model_version=None` raises ValueError per §A1.1. CLI surface: `--corpus-id`, `--window-start/--window-end`, `--model-version`, `--dry-run`, `--max-decisions`, `--folds`, `--resume`. Two follow-ups intentionally out of scope: `parser_strategy_succeeded` stored as None (#98), `enrichment_pit_warnings` stored as () (#99). 11 new tests. PR #876.
- **#96.3 + #96.4 backtester + walkforward consume corpus** (Sprint 1.C Phase 4 sub-tasks 3+4 bundled) — `backtest_model` and `run_walkforward` gain optional `corpus_id: str | None = None` keyword parameter. When set: backtester loads `load_entries_by_decision(corpus_id, parse_clean_only=True)` once at function entry per §A1.4 binding row filter and per-candidate looks up `(date_str, ticker)` via new `_resolve_corpus_decision` helper — entries with `llm_action='taken'` produce trades with their corpus `llm_conviction` recorded; rejected/conviction_none/parse_failed entries skip silently; missing entries logged + skipped (NO live-LLM fallback per §A3 reproducibility). Walkforward gains `_gate_corpus_or_raise` firing §A3 admissibility BEFORE any fold runs + window-coverage gate. Result dict gains `corpus_id`, `manifest_admissibility`, `parse_failed_excluded` keys. When `corpus_id=None`, runtime behavior preserved exactly. 18 new tests. PR #877.
=======
- **#82 deterministic-ranker shadow portfolio** (Sprint 1.C Phase 5) — `src/evaluation/backtester.py::backtest_model` gains optional `shadow: bool = False` keyword parameter; `src/evaluation/walkforward.py::run_walkforward` gains optional `with_shadow: bool = False` keyword parameter. Implements pre-reg §6 secondary diagnostic + addendum §A1.6 "deterministic-ranker shadow uses same prompt minus LLM step" verbatim. When `shadow=True` with `corpus_id` set: every parse_failed=0 corpus entry produces a trade regardless of `llm_action` — strips the LLM filter, leaving the deterministic ranker's selection alone. When `shadow=True` with `corpus_id=None`: every ranker candidate becomes a trade (no LLM call). The §A1.6 fair-comparison rule (parse_failed=0 binding for BOTH primary and shadow) is enforced by reusing `load_entries_by_decision(parse_clean_only=True)` upstream — parse_failed=1 entries are filtered identically on both portfolios so primary minus shadow measures LLM contribution rather than data-cleanliness drift. `_resolve_corpus_decision` gains a `shadow` keyword that toggles the llm_action filter while preserving the parse_failed and corpus-presence guards. When `with_shadow=True`, `run_walkforward` runs BOTH portfolios over the SAME `compute_fold_boundaries` (binding §A1.6 invariant — re-uses the helper, doesn't re-implement) with the SAME corpus + SAME admissibility gate, then composes a delta dict `{primary_excess_sharpe, shadow_excess_sharpe, delta_excess_sharpe, primary_total_pnl_pct, shadow_total_pnl_pct, delta_total_pnl_pct, primary_n_trades, shadow_n_trades}` where `delta = primary - shadow` on each axis. Result-dict shape changes to `{"primary": ..., "shadow": ..., "delta": ..., "corpus_id": ..., "manifest_admissibility": ...}` only when `with_shadow=True`; `with_shadow=False` (default) preserves the existing flat dict shape exactly (regression-lock for #81 subgroup-analysis + every other consumer). 11 new tests in `tests/evaluation/test_shadow.py` covering: shadow path takes every parse_failed=0 entry regardless of action; primary still filters to llm_action='taken' (regression); both portfolios filter parse_failed=0 (§A1.6 binding); shadow trade count >= primary trade count by construction; shadow without corpus takes every ranker candidate; with_shadow=True returns dict with primary+shadow+delta keys; with_shadow=False preserves flat shape; both portfolios receive identical fold boundaries; delta arithmetic correct on deterministic fixture; shadow=True propagated to per-fold backtest_model call; with_shadow=True still surfaces top-level corpus provenance. Sibling-search receipt: `backtest_model` callers in `cli/commands.py:753` (positional model+months — unaffected by new keyword), `compare_models` in `backtester.py` (positional — unaffected), `walkforward._run_fold` (updated to forward shadow=); `run_walkforward` callers in `walkforward.main` (CLI defaults False — unaffected) + `tests/evaluation/test_walkforward.py` (with_shadow defaults False — unaffected); `scripts/generate_llm_corpus.py` only imports `compute_fold_boundaries` + `_is_trading_day` (not run_walkforward — unaffected); `src/platform/rigor/walkforward*.py` are a separate subsystem (different signature, not affected). `src/evaluation/walkforward.py` 367→427 lines added to `config/known_violations.json` with rationale (real refactor — three small helpers + nested-helper compression — already applied; further extraction would split tightly-coupled fold-orchestration code across files). Pairs with #96.3+#96.4 corpus wiring; closes Phase 5. Phase 6 (Stage 1 backtest run) is operator-present per binding instruction.
<<<<<<< HEAD
- **#96.2 LLM-scoring corpus generator** (Sprint 1.C Phase 4 sub-task 2 of 4) — `src/evaluation/corpus_generator.py` + `scripts/generate_llm_corpus.py`. `generate_corpus(corpus_id, decision_points, features_by_date, model_version, config, code_sha, window_start, window_end, dry_run=False, resume=False)` enumerates walk-forward decision points, calls `enrich_features(..., as_of=as_of)` for PIT-clean enrichment per §A2.1, builds the prompt via `_build_feature_prompt` (frozen at v0.32.0 per §A1.3), calls `enhance_packet_with_llm`, captures `llm_conviction` + `llm_conviction_parse_failed` from the resulting packet, and streams `CorpusEntry` rows to `entries.jsonl` followed by a final `manifest.json`. `model_version=None` or empty string raises ValueError per §A1.1 (corpus is bound to one model version; default in CLI is `arcis:v1.0.0`). Section 8 (options) + Section 11 (cross-asset) emit `n/a` placeholders naturally — recorded as `prompt_section_omitted=(8, 11)` on every entry + `section_pit_status[8]="placeholder"` + `[11]="placeholder"` in the manifest, matching addendum §A1.3 + §A2.2. CLI surface: `--corpus-id`, `--window-start/--window-end`, `--model-version`, `--dry-run` (skips LLM, plumbing test only), `--max-decisions`, `--folds`, `--resume` (skips entries already on disk by (as_of, ticker) key — multi-hour runs survive crash mid-run). Decision-point enumeration reuses `src/evaluation/walkforward.py::compute_fold_boundaries` + `src/universe/pit.py::get_sp100_at` (sibling-search receipt). Two follow-ups intentionally out of scope per dispatch — `parser_strategy_succeeded` stored as None (packet_writer doesn't track which conviction-parse strategy fired) and `enrichment_pit_warnings` stored as () (fetchers don't emit warnings lists yet); both filed as trackers. 11 new tests in `tests/evaluation/test_corpus_generator.py` covering generator core path, parse_failed round-trip, as_of forwarding to enricher, dry-run path skips LLM, resume skips existing keys, section_pit_status records placeholder for 8+11, admissibility matches `compute_admissibility()`, ValueError on missing model_version, integration with `iter_entries`, and CLI `--help` smoke test. Builds on #96.1.
=======
- **#96.3 + #96.4 backtester + walkforward consume corpus** (Sprint 1.C Phase 4 — sub-tasks 3+4 bundled) — `src/evaluation/backtester.py::backtest_model` and `src/evaluation/walkforward.py::run_walkforward` gain optional `corpus_id: str | None = None` keyword parameter. When set: backtester loads `load_entries_by_decision(corpus_id, parse_clean_only=True)` once at function entry (per pre-reg §A1.4 binding row filter) and per-candidate looks up `(date_str, ticker)` via new `_resolve_corpus_decision` helper — entries with `llm_action='taken'` produce trades with their corpus `llm_conviction` recorded on the trade dict (§A1.5 + §A3.1 round-trip integrity); rejected/conviction_none/parse_failed entries skip silently; missing entries logged + skipped (NO live-LLM fallback per §A3 reproducibility). Walkforward gains `_gate_corpus_or_raise` helper firing the §A3 admissibility gate BEFORE any fold runs (RuntimeError on non-admissible manifest) plus window-coverage gate (RuntimeError if any fold falls outside `manifest.walkforward_window_start..walkforward_window_end`); `corpus_id` propagates to each fold's `backtest_model` call. Result dict gains additive `corpus_id`, `manifest_admissibility`, `parse_failed_excluded` keys for downstream provenance (#81 subgroup analysis stays compatible — additive only). CLI gains `--corpus-id` flag. When `corpus_id=None`, runtime behavior is preserved exactly (regression-locked). 18 new tests across `tests/evaluation/test_backtester_corpus.py` (NEW, 6 tests) + extended `tests/evaluation/test_walkforward.py::TestRunWalkforwardCorpus` (12 tests). Pairs with #96.2 generator (parallel sibling). Depends on #96.1 contract (PR #875).
>>>>>>> 86dc4a6 (feat(evaluation): backtester + walkforward consume LLM-scoring corpus (#96.3 + #96.4 Phase 4))
>>>>>>> 1b57f8e (feat(evaluation): deterministic-ranker shadow portfolio (#82 Sprint 1.C Phase 5))
