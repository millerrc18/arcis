...
## [v0.36.55] — 2026-05-23 — CI workflow hygiene (#101): PR-only path filter on lifecycle-smoke, installed-packages cache on pg-tests + lifecycle-smoke, replace failing auto-submission with scoped main-only dep-submission

W21 phase-4 first early-wedge per the operator's attack-order (memory: `project_w21_attack_order`). Lands BEFORE the remaining hotfix backlog (#92 / #100 / #86 / #51 / #77) so each downstream PR benefits from the optimized CI throughput. No application-code changes — pure workflow surgery across `.github/workflows/`.

- **lifecycle-smoke.yml: PR-trigger path filter, push:main unfiltered.** `on.pull_request` now restricts to `paths: ['src/simulation/**', 'tests/simulation/**', '.github/workflows/lifecycle-smoke.yml']` — most PRs don't touch the simulation pipeline, so the ~9.5min smoke job is now skipped on those. `on.push: branches: [main]` deliberately has NO `paths:` filter — every merge to main still runs the smoke as the safety net for upstream-refactor breakage (the smoke imports from `src/shadow_trading`, `src/schema`, `src/scheduler`, `src/risk`, `src/cost_model`, `src/llm`, `src/data_ingestion`, `src/universe`, `src/journal` — all upstream of `src/simulation/` at import time; a refactor to any of them that breaks the lifecycle would otherwise only be caught by the nightly lifecycle-full-gate, leaving a 24h reliability window during which #95 wipe-gate trust degrades). The asymmetry is intentional per the operator's strict-rigor policy: catch upstream-refactor regressions at MERGE TIME, not 24h later.
- **pg-tests.yml + lifecycle-smoke.yml: installed-packages cache (`env.pythonLocation`).** Both workflows now add an `actions/cache@v4` step keyed on `${{ runner.os }}-python3.12-pkgs-${{ hashFiles('requirements.txt') }}` with path `${{ env.pythonLocation }}` (the setup-python install root — captures site-packages + .pth files + entry-point scripts). Complementary to setup-python's existing `cache: 'pip'` (which caches the wheel download cache at `~/.cache/pip`): the two together give cold-miss ~60-90s + warm-hit ~5s for the dep-install step. Key components include `python3.12` (so a future 3.13 bump doesn't poison the cache — `.so` files are cpython-version-specific) and `hashFiles('requirements.txt')` ONLY — `requirements-cloud.txt` (server-only) and `requirements-training.txt` (training-only + volatile unsloth git+URL) are NOT installed in these CI jobs, so hashing them would force unnecessary cold-cache rebuilds. The lifecycle-full-gate job inside pg-tests.yml gets the same cache step (shared key means it can share entries with the pg-tests job when the requirements.txt hash matches).
- **dependency-submission.yml: replaces failing auto-submission, scoped to main+requirements.txt.** GitHub's auto-generated "Automatic Dependency Submission (Python)" workflow scans ALL `requirements*.txt` files unconditionally; `requirements-training.txt`'s `unsloth[cu128-torch260] @ git+https://github.com/unslothai/unsloth.git` dep breaks the upstream parser, causing submit-pypi to fail on every PR push (1163, 1164, 1165, 1166 all RED; 1162 only went green by chance after a later push). New file `.github/workflows/dependency-submission.yml` runs `pypa/gh-action-pip-audit@v1.1.0` on `push:main` ONLY (skip per-PR entirely) with `inputs: requirements.txt` — the explicit scoping sidesteps the unsloth URL by NOT scanning the training/cloud files. Job is `continue-on-error: true` so vuln findings on base deps don't block main pushes (Dependabot alerts are the real surface). To fully suppress the auto-submission, operator runs `gh api -X PATCH /repos/:owner/:repo -F 'security_and_analysis[dependency_graph_autosubmit][status]=disabled'` once post-merge (documented in PR description).
...
**Net effect at ~30 pg-tests runs/week + ~20 lifecycle-smoke runs/week + ~30 submit-pypi runs/week:**
- Without changes: ~50 dep-install runs × ~75s = ~62 min/week dep-install + ~30 submit-pypi × 3min = ~90 min/week submit-pypi + ~14 lifecycle-smoke skips × 9.5min = ~133 min/week lifecycle-smoke = **~285 min/week (~1200 min/month) CI consumption**
- With v0.36.55: cache warm-hit ~95% → ~7 min/week dep-install + ~5 submit-pypi (main only) × 3min = ~15 min/week submit-pypi + lifecycle-smoke skips on ~70% PRs = ~95 min/week saved on smoke = **~115 min/week consumption (~485 min/month)**
- **Estimated savings: ~720 min/month — ~60% reduction.**
...
2. Re-trigger the same workflows: cache HIT. Install drops to ~3-5s (check the "Cache" step log for `Cache restored from key: ...`).
3. No-op edit to `requirements.txt` (trailing newline) on a test branch: cache MISS (different hash). Confirms invalidation.
4. PR touching only docs / non-sim code: lifecycle-smoke SKIPS at PR-trigger time (the visible bulk savings); merge-to-main still re-runs it as the safety net.
5. After merge, run `gh api ... dependency_graph_autosubmit.status=disabled` once to fully suppress the auto-submission. Then the next push:main fires ONLY the new `submit-pypi` job (green) and the auto-generated one stops appearing.

**Read first when modifying CI workflows:** `feedback_use_coding_team_skill` (this is a single-tracker hotfix, direct dispatch fine), and `feedback_strict_rigor_no_handwave` (verification steps above are mandatory before claiming the savings).
