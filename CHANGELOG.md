...
- **docs(operator-guide): 4 post-PR-#936 reviewer enhancements.** §1 Quick Start prerequisites table gains a Hardware row (NVIDIA GPU ≥12 GB VRAM; RTX 3060 current / RTX 3090 planned; Ollama inference + corpus gen throughput gated). §5 Troubleshooting gains two new decision trees: "Corpus generation appears stalled" (progress check, VRAM/rate-limit/NSSM/checkpoint failure causes, idempotent `--resume` recovery) and "Watchdog has timed out" (watchdog.txt timestamp check, process detection, clean-restart steps; cross-ref §6). §8 Glossary gains SD#NN entry defining the Strategy Decision convention with SD#33/41/43/46 as canonical examples and a pointer to MASTER.md §5 + `docs/research/SD-<NN>-*.md` research memos.
