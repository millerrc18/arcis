...
- **#856 fundamentals PIT routing** (Sprint 1.C Phase 2 follow-up #3 of 5) — `src/data_enrichment/fundamentals.py::fetch_fundamental_snapshot` gains an `as_of: str | None = None` parameter. The two private helpers `_get_latest_value(concept_data, form_filter, as_of=None)` and `_get_ttm_value(concept_data, as_of=None)` now filter XBRL entries by `filed <= as_of` BEFORE sorting, then sort by `filed` desc primary / `end` desc secondary. Without the fix, code sorted by `end` (period covered) — at as_of=2024-12-31 it would return Q4 (end=2024-12-31, filed=2025-02-15) even though the 10-Q wasn't filed until Feb 2025. Cache filename encodes `as_of` so PIT and runtime caches don't collide (`{ticker}_fundamentals.pkl` vs `{ticker}_fundamentals_asof_{as_of}.pkl`). When `as_of` is None (runtime), legacy sort-by-`end` behavior is preserved. `src/data_enrichment/enricher.py::enrich_features` routes its `as_of` parameter through to `fetch_fundamental_snapshot`. 10 new tests in `TestFundamentalsAsOfRouting` lock the routing contract, including the synthetic-XBRL fixture that exercises the bug-vs-fix divergence at as_of=2024-12-31.
