...
- **#854 news PIT routing** (Sprint 1.C Phase 2 follow-up #1, cheapest of the must-fix bundle) — `src/data_enrichment/enricher.py::enrich_features` gains an `as_of: str | None = None` parameter. When set, news fetch routes to `fetch_historical_news(as_of_date=...)` (already implemented at `news.py:200`, just not previously wired). When None (runtime default), behavior unchanged. 3 new tests in `TestEnrichFeaturesAsOfRouting` lock the routing contract. Establishes the `as_of` plumbing pattern that #855 (macro+cross-asset), #856 (fundamentals), #857 (insiders), #859 (earnings_signals) will follow.
- **#855 macro PIT routing** (Sprint 1.C Phase 2 follow-up #2 — Section 7 of the LLM prompt) — `src/data_enrichment/macro.py::fetch_macro_context` and the internal `_fetch_series` / `_fetch_cpi_yoy` helpers gain an `as_of: str | None = None` parameter. When set, FRED API calls send `observation_end=as_of` so the most-recent observation returned is the one available at that historical date (FRED supports PIT lookups natively — infrastructure was there, just unwired). Cache key now encodes `as_of` (`macro_context_<YYYY-MM-DD>.pkl` vs `macro_context.pkl`) so PIT and runtime data don't collide on disk; PIT cache entries skip the freshness check since historical FRED values are immutable. `enrich_features` forwards its `as_of` through to `fetch_macro_context`. When `as_of=None` (runtime default), behavior is byte-identical. 6 new tests in `TestMacroAsOfRouting` lock the FRED PIT-routing contract. Section 11 (cross-asset: `us_10y_yield`/`dxy_level`/`hy_oas`/`gold_change_1m`) was bundled with this fix in the audit, but investigation found NO live producer in `src/` for those features — `packet_writer.py` reads them via `features.get(..., 'n/a')` and they always render `n/a`. PIT plumbing for Section 11 is therefore not applicable until a fetcher is added; tracked as separate follow-up.
