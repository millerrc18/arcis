...
<!-- T4c --> Migrated training+risk caller sites from `try/except Exception { import + check + notify_X }` to `safe_send`: `training/canary.py` `_send_alert` (→ `safe_send("model_event", ...)`), `training/trainer.py` `export_training_data` holdout-empty alert (→ `safe_send("trainer_holdout_empty", ...)`), `risk/governor.py` `_warn_governor_disabled_once` (→ `safe_send("system_event", ...)` with `is_telegram_enabled()` pre-check to preserve idempotency test). `training/ingestion_gate.py` `alert_training_halt` NOT migrated: existing test `test_alert_training_halt_includes_reason_hint` patches `send_telegram` without patching `is_telegram_enabled`; migrating to `safe_send` (which short-circuits on disabled telegram) would break that test and the test file is not in T4c scope.
<!-- T4d --> Migrated misc caller sites to `safe_send`: `research_synthesizer.py` (`send_telegram` → `safe_send("research_digest", ...)`), `cli/commands.py` `_notify_startup_telegram` (`notify_startup_complete` → `safe_send("startup_complete", ...)`). `api/cloud_routes/platform.py`: removed outer `try/except Exception` wrappers around `platform_events.notify_*` calls (platform_events already handles network failures internally via `_send()`). `evaluation/auditor.py`: no Telegram call sites — no changes (only `send_email` paths).
