...
- **fix(#919 follow-up): apply reconciled_stale exclusion to cto_report and strategy_detail routes** — Operator caught two more dashboard tiles still showing pre-#919 win-rate after #919 deployed (the "section above scan_metrics"). Sibling-search of `src/api/cloud_routes/analytics.py` revealed two routes where outcome aggregations on closed shadow_trades did not apply `outcome_stats_filter_sql()`: (1) `cto_report` (line 408) — headline_kpis, by_score_band (480), by_sector (506), by_regime (519), execution_analysis (532), fund_metrics monthly_batting_avg (587), and by_broker (605); (2) `strategy_detail` (749) — per-strategy trade list. Fix: in `cto_report`, build `closed_recent_for_stats = [t for t in closed_recent if not t.get('exit_reason') or t['exit_reason'] not in EXCLUDED_FROM_OUTCOME_STATS]` and route ALL outcome aggregations through it — the by_exit_reason histogram (line 457) intentionally stays on the unfiltered `closed_recent` so reconciled_stale shows as informational signal. In `strategy_detail`, append `outcome_stats_filter_sql()` to the existing WHERE clause directly (no histogram concern at this route). No new tests added: the underlying primitives (constant + SQL fragment) are already locked by 6 tests in `tests/shadow_trading/test_exit_reason_taxonomy.py` from #919; the new in-memory list-comprehension is a 1-line application of the same constant. After Render redeploy: dashboard CTO-report panel and per-strategy-detail page reflect the same 100%-win-rate truth as the top KPI strip. Sibling-sweep also confirmed `attribution_stats` reads from a different table (`attribution_trades`, with explicit ranker_only_outcome / llm_portfolio_outcome columns) — not affected by this class of bug.
