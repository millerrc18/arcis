...
- **#857 insiders PIT routing** (Sprint 1.C Phase 2 follow-up #2 of the must-fix bundle) — `src/data_enrichment/insiders.py::fetch_insider_activity` gains an `as_of: str | None = None` parameter. When set, computes `[as_of - lookback_days, as_of]` window and passes `from`/`to` to the Finnhub insider-transactions API; the cache key is namespaced by `as_of` so PIT and "now" data don't collide; client-side filter additionally drops post-`as_of` transactions as defense-in-depth. When None, runtime behavior unchanged. `_fetch_from_finnhub` and the `_get_cache_path` / `_load_cached` / `_save_cache` helpers are extended to thread `as_of` through. `enricher.py::enrich_features` routes its existing `as_of` arg to the insider fetch site (mirrors the #854 news pattern). 5 new tests in `TestInsidersAsOfRouting` cover: default preserves runtime, `as_of='2024-06-15'` sends `from='2024-03-17'`/`to='2024-06-15'`, transaction filter drops out-of-window/post-as_of records, cache key includes `as_of`, enricher passes `as_of` through. **Coverage limit:** Finnhub free-tier insider history covers ~2-3 years; pre-reg addendum 1 §A4 confirms 2023-09 OOS start is inside that window but earliest folds may be sparse.
