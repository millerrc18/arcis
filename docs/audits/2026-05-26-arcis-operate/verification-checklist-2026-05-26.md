...
1. **`gpu-degraded.md` duplicate risk keys (RESOLVED)** — The initial T6 commit shipped with both `risk: medium` and `risk-level: medium` in the frontmatter. Spec §4 validator check (a) enumerates `risk-level` as the only canonical required key — `risk:` alone is non-canonical and would fail the DA7 validator. Followup commit `e21b9d14` dropped the redundant `risk:` from `gpu-degraded.md`, and cycle-1 dual-Opus QA fix commit `402893f6` then corrected the same drift in `watchloop-wedged.md` (which had shipped with only the non-canonical `risk:` key). All 5 runbooks now use canonical `risk-level:`; `tests/test_operate_runbook_frontmatter_consistency.py::test_no_runbook_uses_legacy_risk_key` regression-locks the canonical spelling.
