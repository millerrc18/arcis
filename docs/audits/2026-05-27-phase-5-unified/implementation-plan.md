...
- **PR-E2 LANDED 2026-05-29** commit `dbef0f2b` (PR #1193) — suite green-gate (#102b, T40-T44): drove the full PG-aware suite to GREEN — **6992 passed / 35 DD-42-justified skips / 4 xfailed / 0 failed / exit 0 / gate clean / 0 xpass** (local authoritative; CI Actions-billing-blocked, merged on operator decision "see what happens"). 38 commits. **T43 gate sentinel** (`tests/test_suite_integrity.py` + conftest `pytest_sessionfinish`/`pytest_runtest_logreport` + `pytest.ini xfail_strict`): fails on any fired skip outside the 5 DD-42 categories or any XPASS; semantic matcher + 23 self-tests; mutation-verified. ~23 genuine failures root-caused incl. **2 real SUT bugs** — `ensure_columns` NOT-NULL `ALTER ADD` migration default (`src/schema/sqlite.py`) + email_digest INSERT-OR-IGNORE→engine_aware_upsert; stale tests repaired via PR-C refactor-retargets (no assertions weakened), fixture-bug fixes, now-relative seed dates, safe_send dispatch-boundary mocks; `.gitattributes` shell-LF pin (CRLF broke the pre-push hook); CI pg-tests 5434-PG service + pytest-asyncio; restored over-deleted `test_site18`; made `delete_insert` cascade test non-vacuous. **dual-Opus QA Attempt 1: B SOUND(93%), A FINDINGS(2) — BOTH convergently CAUGHT a self-inflicted regression** (wave-5k digest confidence `*100` was a mis-diagnosis — `confidence_weighted_score` is stored 0-100 per aggregation.py:237, old code correct; reverted SUT + fixed the test's bad "0.73" seed). A also caught an inaccurate lifecycle skip reason → made honest. Both findings fixed; green re-confirmed post-QA. **Tracked residual (NOT product bugs/regressions): GitHub #1192 + kin #30** — 9 test-isolation defects (6 order-dependent + 3 run_smoke env-scrub) carry DD-42 cat-4 `tracked-upstream-bug(#1192)` skips; per-victim bisection scoped there. Line-pinned raw-connect allowlist drifted 4× (content-key follow-up; memory reference_line_pinned_allowlist_fragility). **`Closes #126` in the PR body was a STALE/WRONG ref — it auto-closed the unrelated EDGAR accession bug #126; coordinator REOPENED #126 immediately + annotated.** Squash-merged to main; branch retained for #1192 follow-up.
- **PR-F LANDED 2026-05-30** commit `51673221` (PR #1194) — docs consolidation (#99-docs) + v0.36.78 version cut: T31-T36 + #6-fix shipped as 10 commits (T31 README rewrite `88e5f9b4`; T32 MASTER §2 rolling-window DD-24 + DIRECTORY regen + `generate_directory.py` git-ls-files tracked-filter + `len(TABLES)` + de-hardcoded 5 stale annotations `69db7c55`; T33 CLAUDE delta + CHANGELOG 6-marker unification into `## [v0.36.78]` + RELEASES cross-links DD-25 + version cut `87fb6320`/`17fbb31a`/`9040f690`/`d8186f88`; T34 git-mv archive sweep 168 files/50 receipts -> docs/archive/sprint-receipts/, 41 png sha256-identical DD-31, history preserved `d6cc0572`; T35 render-decommission Phase-4 receipt + standards/runbooks header sweep `77032b5a`; T36 2 non-grandfathered sentinels + 2 verify-by-mutation companions `c18ae477`; #6 follow-up fix `0eb0e72a`). **v0.36.78 cut OPERATOR-CONFIRMED (AskUserQuestion)** — policy-consistent per docs/versioning-policy.md: version.py + README badge + test_version.py all v0.36.78 (coupling-traced: no frontend version-constant; Layout.jsx fallback='unknown'); coordinator creates+pushes the tag at PR-G/T39. **T32 coordinator close-out** of a session-interruption pre-commit cutoff (verify-by-mutation + reproduce-check + count cross-checks; PR-E T29 precedent — sound reproducible work verified, not discarded). dual-Opus QA Attempt 1 BOTH SOUND 100% (A: docconsistency consumer degrades gracefully, no runtime break; B: blob-hash-PROVED failing-test source identical origin/main vs branch + refined quiet-hours root cause to an unprovisioned `notifications_digest_queue` table). **FAILURE DISAMBIGUATION** (feedback_ci_red_disambiguation; CI quota-dark): run1(clean)=6 failed = #6 (PR-F: T34 archived the dir test_scan_service_regime_logging path-asserts; FIXED `0eb0e72a`) + 5 quiet-hours-flaky alert tests (PRE-EXISTING, proven on origin/main: safe_send policy-gate routes system_event->digest at quiet-hours hitting an unprovisioned digest-queue table). run2 (contaminated by a concurrent QA worktree) added test_resolver (PRE-EXISTING midnight date-boundary flake, proven identical-fail on main) + connect_db_complete (CONTAMINATION: QA-B tmp/qaB-mainwt worktree scanned; PASSES clean post-cleanup). PR-F = ZERO regressions. **Kins**: quiet-hours-flaky alert tests x5 (main is night-RED; PR-E2 '0 failed' was daytime), test_resolver midnight date-boundary flake, T34 stale audit-path citations (19 refs / 20 files incl 7 #1192-scanned src — deferred to avoid allowlist drift), test_no_raw_sqlite3_connect doesn't exclude tmp/+worktrees, WhatsNewPanel.jsx v0.32.0 drift, ~11 legacy deep CHANGELOG [Unreleased] headers. Non-mutating (only src touches = version.py constant + generate_directory.py, both zero-runtime-risk) — merged immediately post-QA, no dead-window. **PR-G #125/#126 PRE-MAPPED**: Phase-5 '#125/#126' are INTERNAL kins (kin#125=lazy-import-6-callers = T39 target; kin#126=walkforward already absorbed by PR-E2) colliding with unrelated GitHub bug issues — T39 must avoid 'Closes #N' auto-close + surface document-close-vs-refactor to operator.
- **PR-G LANDED 2026-05-30** commit `f08b53fe` (PR #1195) — Phase-5 CLOSE-OUT + **tag v0.36.78 PUSHED** (annotated `308309df` -> `f08b53fe`): T37 (`48bfe9f6`) 3 non-grandfathered close-out sentinels (known_violations freshness + docs/audits archive-policy [git-tracked] + WatchLoop._safe_run->CollectorResult AST contract) + 3 verify-by-mutation companions + the enabling known_violations prune (2 now-undersized rows: kpis_compute.py 401->366L [PR-B Render strip], src/training/backfill.py stale 343->344L; KC-6 cloud_routes + grandfathered >400L PR-C residuals RETAINED); T38 (`1e7ca722`) CHANGELOG ### PR-G in the ## [v0.36.78] block; T39 (`621bbe77`) RELEASES v0.36.78 phase-5 receipt + Cleanup-1 kin subsumption; + path-fix (`e454edf6`) + minimal-diff re-serialize (`e14e260d`). **125/126 NUMBERING-TRAP handled**: Phase-5 125/126 = INTERNAL kins (kin-125 lazy-import DOCUMENT-CLOSED, deferred to Phase-6 — not a green-gate blocker; kin-126 walkforward ABSORBED by PR-E2/T44), NOT the unrelated GitHub bug issues (GH-125 options-NaN CLOSED, GH-126 EDGAR OPEN, both stay correct); T38/T39 dev self-caught + remediated a message-narration slip (reworded+force-pushed); coordinator trap-safed the PR body; ZERO Closes-keyword + zero bare-hash-issue refs in shipped content. **dual-Opus QA Attempt 1 BOTH SOUND 100%**: A semantic-JSON-diff'd the prune (exactly 2 rows, 56->54) + ran the sentinel suite (10p); B EMPIRICALLY proved non-vacuity (mutated real watch.py annotation + real allowlist -> predicates flip) + blob-hash-proved the 6 run-failures pre-existing. Both flagged a known_violations Unicode-churn nit -> coordinator fixed (minimal-diff re-serialize). **AUTHORITATIVE run3 (CLEAN, no concurrent agents)**: 6 failed / 6996 passed / 35 justified-skip / 4 xfail / 0 xpass — the 6 = EXACTLY the documented pre-existing night-flakes (4 governor + 1 ingestion quiet-hours + 1 test_resolver date-boundary; blob-hash-identical src on both refs); PR-G's 3 sentinels + T36 sentinels PASS; connect_db_complete PASSES clean (PR-F-run2's failure was QA-worktree contamination, not allowlist drift). PR-G = ZERO regressions. Non-mutating — merged immediately post-QA. New minor kin: known_violations RECORDED line-counts are stale-low for several retained cloud_routes entries (analytics 996/actual 1008 etc.) — tolerated grandfather drift, Phase-6 reconcile. 

## ===== PHASE 5 IMPL CAMPAIGN COMPLETE (2026-05-30) =====
All 8 PRs landed on origin/main: PR-A `a8bf5ff9` / PR-B `75334f27` / PR-C `6e5dbaf4` / PR-D `0702ef0a` / PR-E `55d488e4` / PR-E2 `dbef0f2b` / PR-F `51673221` / PR-G `f08b53fe`. **v0.36.78 tagged** (`308309df`). Net: docs surface consolidated (README/MASTER/DIRECTORY/CHANGELOG/RELEASES/CLAUDE), Render code removed, 7 modules split, 21 collectors on CollectorResult, test suite green-gated (DD-42 + T43 sentinel), 9 phase-5 sentinels added, known_violations pruned. Every PR dual-Opus QA SOUND. Non-mutating campaign (zero trading-behavior change). **Capstone gate #95 (clean-slate wipe) now REACHABLE, gated on sim STABLE verdict.** KNOWN PRE-EXISTING (NOT campaign-introduced): the suite is night-RED from a time/date-flake class (quiet-hours safe_send->digest hitting an unprovisioned notifications_digest_queue table + relative-date seed boundaries); PR-E2's '0 failed' baseline was a daytime run. Kin filed for test-determinism fix.
