...
| 21 | **Retry counter persisted to `data/notification_retry_state.json`** (v2 — M4) | In-memory counter resets on every NSSM watch-loop restart. During sustained outages spanning restarts, escalation would never fire. JSON file mirrors existing `data/drift_detector_state.json` pattern. Atomic write via tmp+os.replace. | DB table (rejected: schema bloat for 1-row state); ignore restart durability (rejected: defeats M4); in-memory with singleton lock (rejected: complexity) | Trivial (file path change; module-init read; per-write atomic replace) | Architect (devil's-advocate M4) |
...
| 23 | **New `src/monitoring/` package for operational alerting** (v2 — FEAS2) | `src/diagnostics/` is dedicated to statistical methodology per its `__init__.py` docstring. Operational alerting (drift detection, alert silence) is a distinct category. New `src/monitoring/` package separates concerns. | (a) `src/diagnostics/operations/` subpackage (rejected: docstring still lies); (c) Accept mixing with doc note (rejected: docstring lie); (b — CHOSEN) new top-level package | Low (file moves; no API breakage; import paths in watch.py + tests updated) | Architect (devil's-advocate FEAS2) |
