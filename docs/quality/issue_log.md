...
- **Fix:** Added root `.htaccess` with IONOS-style 301 redirect rule and project domain `halcyonlab.app`.
- **Evidence:** `cat .htaccess` shows rewrite engine enabled, port-443 condition, and redirect to `https://halcyonlab.app/$1`.
