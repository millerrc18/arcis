...
**Decision:** Halcyon Lab adds an optional llama.cpp + GBNF path for grammar-constrained commentary generation, while keeping the existing Ollama path as the default fallback. The feature stays off by default until local validation is complete.
