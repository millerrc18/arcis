...
| `src/scheduler/holidays.py` — `subtract_trading_days` | Sprint 1.A Wave 2 (#911, #922, #923) | NYSE-calendar-aware trading-day arithmetic helper. Signature: `subtract_trading_days(anchor: date, n: int) -> date`. Calendar source: `pandas_market_calendars` (NYSE). Use cases: walkforward fetch anchor (200 trading days = exactly slice_to_date row gate semantic), corpus generator anchor (`scripts/generate_llm_corpus.py`), backtester fetch anchor (`src/evaluation/backtester.py`). Why: replaces 365-calendar-day approximation that drifted with holiday density — 291–293 calendar days = exactly 200 trading days depending on the rolling window. Locked NYSE calendar test in `tests/scheduler/test_holidays.py` catches calendar-rule changes. **Do NOT use `timedelta(days=365)` or other calendar-day approximations for fetch anchors** — use this helper instead (see #888 / #106 incident). |
