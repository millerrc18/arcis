...
def _mock_run(stdout: str = "", stderr: str = "", returncode: int = 0):
    ...
    return patch(
        "src.tools.gitarchaeology._helpers._subprocess_run",
...
def test_log_path_filter():
    ...
    with patch("src.tools.gitarchaeology._helpers._subprocess_run", side_effect=_capture):
...
def test_log_limit():
    ...
    with patch("src.tools.gitarchaeology._helpers._subprocess_run", side_effect=_capture_default):
        log()
    with patch("src.tools.gitarchaeology._helpers._subprocess_run", side_effect=_capture_override):
        log(limit=10)
...
def test_blame_invalid_range(tmp_path):
    ...
    with patch("src.tools.gitarchaeology._helpers._subprocess_run") as mock_run:
