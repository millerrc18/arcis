...
# ── SQLite routing for _fetch_closed_trades (post-Render decommission) ────────
# DATABASE_URL PG branch removed Phase 5 §3.2 (Render decommissioned 2026-05-18).
# _fetch_closed_trades always uses journal.store.get_closed_shadow_trades (SQLite).

class TestFetchClosedTradesPostgresRouting:
    def test_uses_sqlite_regardless_of_database_url(self, monkeypatch):
    ...
    def test_get_kpis_uses_sqlite_data(self, monkeypatch):
        """End-to-end: get_kpis always pulls from journal.store (SQLite path)
        regardless of DATABASE_URL (Render PG decommissioned 2026-05-18)."""
