...
3. Call src.sync.render_sync.expire_stale_commands when DATABASE_URL is set
...
def test_expire_stale_calls_helper_when_database_url_set(client, monkeypatch):
    ...
    with patch("src.sync.render_sync.expire_stale_commands", return_value=7) as mock_helper:
        resp = client.post("/api/commands/expire-stale")
    assert resp.status_code == 200
...
def test_expire_stale_helper_failure_returns_500(client, runtime_stub, monkeypatch):
    ...
    monkeypatch.setenv("DATABASE_URL", "postgresql://test")
    with patch(
        "src.sync.render_sync.expire_stale_commands",
