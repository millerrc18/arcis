"""Tests for compute_timeout_status helper in shadow_service.

Called by: pytest (CI)
Calls: src.services.shadow_service.compute_timeout_status
Owns tables: none
Config keys: none
...
def test_unknown_when_duration_days_is_none():
    result = compute_timeout_status(duration_days=None, timeout_days=15)
    assert result["timeout_progress_pct"] is None
    assert result["timeout_status"] == "unknown"


def test_unknown_when_both_none():
    result = compute_timeout_status(duration_days=None, timeout_days=None)
    assert result["timeout_progress_pct"] is None
    assert result["timeout_status"] == "unknown"
...
def test_exactly_80_percent_is_approaching():
    result = compute_timeout_status(duration_days=12, timeout_days=15)
    assert result["timeout_progress_pct"] == 80.0
    assert result["timeout_status"] == "approaching"


def test_exactly_100_percent_is_overdue():
    result = compute_timeout_status(duration_days=15, timeout_days=15)
...
def test_progress_capped_at_999():
    ...
    result = compute_timeout_status(duration_days=10000, timeout_days=1)
...
def test_llm_rejected_fields_both_present_in_service_shape():
    ...
    timeout_info = compute_timeout_status(duration_days=duration, timeout_days=timeout_days)
