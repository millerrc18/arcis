...
wf_per_job = {}
for job_name, job_def in (wf_data.get('jobs') or {}).items():
    job_text = ''
    for step in (job_def.get('steps') or []):
        if 'run' in step:
            job_text += step['run']
    wf_per_job[job_name] = set(extract_tool_invocations(job_text))
...
for verb in ['audit-skills', 'curate-memory', 'test-tools']:
    ...
    if not rb_path.exists():
        continue
    if verb not in wf_per_job:
        continue
    rb_tools = set(extract_tool_invocations(rb_path.read_text(encoding='utf-8')))
    wf_tools = wf_per_job[verb]
    missing_in_wf = rb_tools - wf_tools
    missing_in_rb = wf_tools - rb_tools
