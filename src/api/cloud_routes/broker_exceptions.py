...
Calls: src.utils.db.connect_db (local SQLite)
...
def get_recent_exceptions(limit: int = 50, since_hours: int = 24) -> dict:
    ...
    Opens a local SQLite connection through connect_db(); the closing(...)
    wrapper prevents the file-handle leak from PR #690 B4 that the
    dashboard's 60s auto-refresh would otherwise compound.
    """
    with closing(connect_db()) as conn:
