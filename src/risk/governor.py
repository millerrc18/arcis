...
# T14: the deterministic-audit entry-suppression gate moved to
# ``governor_audit.py``. Re-exported here so ``RiskGovernor.check_trade`` (and
# any external caller that imported ``src.risk.governor.audit_entry_suppression_reason``)
# keeps resolving the symbol unchanged.
from src.risk.governor_audit import (  # noqa: F401
    _AUDIT_ENTRY_SUPPRESSION_LOOKBACK_HOURS,
    audit_entry_suppression_reason,
)
