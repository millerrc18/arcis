...
Calls: yfinance, src.universe.sp100
Owns tables: none
Config keys: none
...
def fetch_cached_ohlcv(ticker: str, start: str, end: str,
    ...
    # Sanitize ticker for filename (BRK.B -> BRK_B, ^VIX -> _VIX)
...
def warm_cache(scenarios: dict, universe: list[str],
    ...
    for name, dates in scenarios.items():
        # Extend range by 60 days before start (for feature lookback)
    ...
    # Also cache SPY and VIX for each scenario
