...
> **This section is a rolling 3-sprint window (DD-24).** Only the three most
> recent sprints are described inline; older sprints collapse to one-liners in
> the table below, with full detail in `CHANGELOG.md` and `docs/archive/`. Run
> `scripts/verify_docs.py` to check the Key Metrics against live system counts.

### Key Metrics

> Counts below are regenerated by `scripts/generate_directory.py` (see
> `DIRECTORY.md` for the authoritative live snapshot). Test floors are quoted
> from `README.md` / `CLAUDE.md` (the single source of truth for floor lineage).

| Metric | Value |
|---|---|
| Release series | v0.36.x — W21 trading-execution-error cleanup + Phase 5 codebase/docs consolidation (run on the local Docker PostgreSQL runtime post-2026-05 cutover). See `CHANGELOG.md` for per-release detail. |
| Runtime DB | Local Docker PostgreSQL (`halcyon-pg`, port 5433). The 2026-05 SQLite→Postgres cutover made Postgres the live runtime; SQLite remains the test-path DB. |
| Model | arcis:v1.0.0 (Qwen3 8B, Q8_0 GGUF); v2.0.0 retrain gated on walk-forward validation |
| Watch loop | NSSM-managed (`ArcisWatchLoop`); 13 scans/day + overnight cycle. Restart via `nssm restart ArcisWatchLoop` (never `python -m src.main startup` directly). |
| Tests | SQLite floor **5,467** (CI tripwire on the local SQLite suite); PostgreSQL CI floor **5,267** (`pg-tests.yml`, deliberately lower for the chronic-failure class). Floor lineage is maintained in `CLAUDE.md` ("Test count must not drop"). |
| Python source files | See `DIRECTORY.md` Quick Stats (auto-generated). |
| Test files | See `DIRECTORY.md` Quick Stats (auto-generated). |
| Dashboard pages | See `DIRECTORY.md` Quick Stats (auto-generated). |
| Research docs | See `DIRECTORY.md` Quick Stats (auto-generated). |
| Schema tables | Authoritative count is `len(TABLES)` from `src/schema/registry.py`, surfaced in `DIRECTORY.md` Quick Stats. (`CLAUDE.md`'s table count is reconciled separately by Phase-5 PR-F T33.) |
| Hardware | RTX 3090 24GB (primary) + RTX 3060 12GB (secondary), Windows 11, Z690, 24/7 operation. |

### Deployed Components

| Component | Status |
|---|---|
| Watch loop (scan + monitor) | LIVE -- 13 scans/day, overnight mode, NSSM-managed |
| Traffic Light | LIVE |
| Risk governor | LIVE -- 8 hard checks + kill switch |
...
| Overnight collectors | RUNNING -- see `src/data_collection/` for the live collector set |
...
| Telegram | LIVE -- gated behind trade_id |
| Intra-day reconciliation | LIVE -- every 15 min during market hours |
| Dashboard (Arcis) | LIVE -- exposed publicly via Cloudflare Tunnel (`ArcisDashboard` NSSM service); dark/light toggle, mobile-responsive sidebar. Render hosting decommissioned 2026-05 (`docs/operations/render-decommission.md`). |
| Simulation engine | LIVE -- 13 regimes, Monte Carlo, traffic-light validation; honest STABLE verdict (v0.36.54, #97) |
| Schema registry | LIVE -- single source of truth (`src/schema/registry.py`); authoritative table count via `len(TABLES)` |
| Halcyon-audit plugin | LIVE -- domain agents + /audit command |
| Automated guardrails | LIVE -- `tests/test_repo_structure.py` (file/function size, repo layout, scratch-at-root, SQLite-at-root) |
| CI on PRs | LIVE -- tests + guardrails + frontend build + PG-aware sweep (`pg-tests.yml`) |
| Composable tooling (Tiers 1-3) | LIVE -- v0.36.62/63/65, Python-API + CLI tools behind SafeOp / SafetyWindowGuard / ProdGuard + JSON-lines audit log |
...
### Runtime DB and `ARCIS_DB_PATH`

The live runtime DB is **local Docker PostgreSQL** (`halcyon-pg`, port 5433) as of the 2026-05 SQLite→Postgres cutover; the watch loop and all collectors write there. SQLite remains the test-path DB.

`src/config/__init__.py` defines `DB_PATH` via `os.environ.get("ARCIS_DB_PATH", str(<repo_root> / "ai_research_desk.sqlite3"))`. The env-var override is how the SQLite path is pointed at an absolute location; the absolute default prevents CWD-dependent resolution. Any `*.sqlite3` file at the repo root is gitignored and is a stale local artifact — remove it rather than backfilling. For test runs use `TEST_DATABASE_URL=…@127.0.0.1:5434` (the `halcyon-pg-test` instance); never set `ARCIS_ALLOW_PROD_PG_IN_TESTS=1` (it disables the P0 guard that protects the 5433 production DB).
...
### Sprint History (rolling 3-sprint window)

> **DD-24:** the three most recent sprints are described inline; older sprints
> are one-liners. Full per-release detail lives in `CHANGELOG.md`; archived
> sprint receipts live under `docs/audits/` and `docs/archive/`.

**Phase 5 — Codebase + Docs Consolidation (current, v0.36.7x series).** The
W21-completion cleanup phase: a 7-PR wave (PR-A…PR-G) that clears repo debris,
adds preventative structure rules, refactors the safest grandfathered files,
migrates the data-collection contract to `CollectorResult`, holds the test
floors (SQLite 5,467 / PG 5,267 per DD-20), and consolidates the doc surface
(README/MASTER/DIRECTORY/CHANGELOG). PR-F (this wave) rewrites MASTER §2 to this
rolling window (DD-24) and regenerates `DIRECTORY.md` from `scripts/generate_directory.py`.
Design: `docs/audits/2026-05-27-phase-5-unified/`.

**Cleanup Sprint 1 (v0.36.71, 2026-05-27, #112–#124).** Observability +
backtest-CLI + simulation/test-infra batch landed as commit-per-fix across a
3-wave dependency-aware structure; included the multi-GPU `_collect_gpu_metrics`
parser fix (#117) and the TradingState `GPU_METRICS` text=date hotfix (#124b).

**Arcis Strategy Skill (v0.36.70, 2026-05-27, #110).** Research-desk capstone:
the `/arcis:strategy` workflow composing the `src/platform/` backtest engine +
rigor stack with specialized + research-team agents; writes only to the local
research DB.

| Older sprint (one-liner) | Ref |
|---|---|
| Email Consolidation PR 1 | v0.36.69 (#115) |
| `/arcis:periodic-discipline` meta-skill — plugin-infra drift auditing | v0.36.68 (#111) |
| `/arcis:operate` skill — incident response + change orchestration | v0.36.67 (#109) |
| Tier 1/2/3 composable tooling (SafeOp / SafetyWindowGuard / ProdGuard + audit log) | v0.36.62 / .63 / .65 (#104–#107) |
| Sim gate-completion — organic open→exit→reconcile lifecycle, honest STABLE verdict | v0.36.54 (#97) |
| CI workflow hygiene — PR-only path filter, package cache, training-reqs relocation | v0.36.55 (#101) |
| SQLite→Postgres cutover follow-ups — boundary tests + install-watchdog + runbook | v0.36.53 |
| Sprint 3 — Cockpit Coherence (22 tasks, header TL source-of-truth, `_meta` envelope) | 2026-05-07 (#983–#1004) |
| Walk-Forward Validation v1 — three-state PASS/FAIL/INCONCLUSIVE, R1-R8 rigor | v0.25.0 |
| Trading-Readiness Audit — Stage-1 honest baseline signed; 3-stage roadmap | 2026-04-27 |
| Track 1.5 — instrumentation gap closure (B1–B9), fix-before-trade principle | v0.27.0 |
| Platform Foundation Sprint 1 — `src/platform/` package, DSR gate, backtest engine | v0.24.0-alpha1 |
| Schema Registry — all DDL centralized in `registry.py`, CI guardrails | #189 |

Pre-v0.24 sprint receipts (Sprints 4A–8, IB-1…IB-7, Dashboard DB-1…DB-FINAL,
production-sweep + hotfix bundles) are archived; see `CHANGELOG.md` and
`docs/archive/` for the full lineage.
