...
| Training data — Stage 1 corpus | Stage 1 corpus generation: **COMPLETE 2026-05-11** (§B2 admissibility PASS, 67,528 entries, SHA256 `43c2e3edb2cd4bb450a890da388ec2ade49ce3205d67a0525f2bb74485606d93`, A3 cold-read verdict PROCEED to walk-forward). Manifest pinned at `data/corpus/stage1-001/MANIFEST.md` per Sprint S1-CC Batch A. |
...
| #530 Sprint A — scheduled-kind find_candidates_for_date wiring (#494) | claude/scheduled-kind-find-candidates-KeLXe | First of 8 prerequisite sprints in the #530 chain unblocking v0.26.0 incumbent YAML extraction (#523). Replaced warn-and-return-`[]` stub in `src/platform/signal_eval.py::find_candidates_for_date` with `_find_candidates_scheduled`: universe ∩ sector_filter ∩ `_matches_scheduled_trigger` ∩ `is_excluded_event_date` ∩ open-position dedup; constant 0.5 signal strength; `metadata.trigger="scheduled"`. Honors `"sp100"` string aliases (which backtest_engine's `_run_scheduled` does not). 10 new tests on fixed 2023-11-06 Monday anchor. Walk-forward path (backtest_engine._run_scheduled) untouched. |
| #530 Sprint B — python_plugin find_candidates_for_date wiring (#493, #548) | claude/scheduled-kind-find-candidates-KeLXe (stacked) | Second of 8 in #530 chain. Replaced `NotImplementedError` stub with `_find_candidates_python_plugin`: dispatches to `plugin_registry.get_plugin(entry.plugin_ref or spec.strategy_id)`, translates `Candidate` dataclass objects → shadow_harness dict shape (adds `trigger="python_plugin"`, `signal_direction`, `plugin_ref`, `strategy_spec_hash` to metadata), preserves plugin-supplied metadata keys. Error contract via built-ins: `KeyError` (no plugin), `RuntimeError` (plugin raised, chained via `from exc`), `TypeError` (bad return shape). Event_exclusion short-circuits BEFORE plugin dispatch (plugin NOT invoked on excluded days). No schema changes — `entry.plugin_ref` is an optional dict key, not a validated field. 13 new tests. Backtest_engine still raises for python_plugin kind (historical replay out of scope); walk-forward path unchanged. |
| #530 Sprint C — scoring-DSL schema block (#549) | claude/extend-scoring-dsl-MyYQl | Third of 8 in #530 chain. Extended `src/platform/strategy_spec.py::validate_spec` with an optional `ranking.bands` block — declarative scoring DSL the Sprint F ranker port will consume in place of the hardcoded bands in `src/ranking/ranker.py::_score_ticker`. Accepts `{metric: str, range: [lo, hi], score: numeric}` with `lo < hi`; rejects missing/non-numeric/inverted ranges, non-numeric scores, and the `isinstance(True, int)` bool-is-int trap. Multiple bands per metric are allowed; overlapping ranges on the same metric emit a `[PLATFORM] ranking.bands overlap` warning via `logger.warning` but still validate (preserves `(ok, errors)` signature — no callers break). **Schema-only sprint, no runtime consumption.** `strategy_spec.py` grew 131 → 195 lines (under 250 cap). 23 new tests in `tests/platform/specs/test_schema_scoring_dsl.py` cover every rejection path + overlap-warn + backward compat on `lazy_prices_v1` and `post_audit_ruleset_v1`. |
| #530 Sprint D — multi-target brackets + regime-adaptive sizing schema (#550) | feat/schema-brackets-sizing | Fourth of 8 in #530 chain. Extended `validate_spec` with two additive blocks: (1) list-form `exit.targets[]` alternative to the legacy singular `exit.target`, with XOR enforcement when `exit.kind == "mechanical"` (`python_plugin` exits pass through with neither), non-empty `atr_multiple > 0`, unique names, and required `exit.stop.atr_multiple` flat-shape; (2) `position_sizing.method: regime_adaptive` with `regimes: {<KEY>: {packet_worthy: bool, position_pct: [0.0,1.0]}}`. Regime-key enum is the incumbent 7-label `classify_regime`/`REGIME_THRESHOLDS` codomain (`BULL_LOW_VOL`, `BULL_HIGH_VOL`, `TRANSITION`, `CORRECTION`, `BEAR_EARLY`, `BEAR_ESTABLISHED`, `CRISIS`); unknown keys warn via `logger.warning` (no translation layer at Sprint F port). Legacy `exit.target` + `fixed_pct_equity` interiors remain unvalidated (backward compat lever — they weren't inspected pre-D). **Schema-only, no runtime consumption.** `strategy_spec.py` grew 195 → 298 lines (under 300 C+D combined cap). 29 new tests in `tests/platform/specs/test_schema_brackets_sizing.py`. Three test-helper fixtures updated from `exit: {kind: mechanical}` to `exit: {kind: python_plugin}` where brackets aren't exercised; production YAML specs untouched. Housekeeping: grandfathered `signal_eval.py` (450 lines) in `config/known_violations.json` — grew past 400-line cap in Sprint B but wasn't added. |
| #530 Sprint E — hooks, enrichment, post-scan, event-risk, bootcamp schema (#551) | feat/schema-final-blocks | **Fifth of 8 in #530 chain — schema surface now complete.** Extended `validate_spec` with five additive optional blocks: `hooks.attribution` (strict, list of logger hook refs), `enrichment.chain` (warn, ordered enricher list), `post_scan.chain` (warn, ordered post-ranking helpers), `event_risk.quarantine_categories` (warn, 20-seed category union of MACRO_EVENT_TYPES + KNOWN_EVENTS labels + sprint-prompt earnings categories), and `bootcamp` (strict, dict with 4 permitted override keys — `qualification_threshold`, `watchlist_threshold`, `max_positions`, `traffic_light_floor`, each type-checked). Per-block policy chosen by registry maturity: strict where the registry is stable and typos are load-bearing (hooks: 2-year-old `src/attribution/logger.py`, 14 import sites; bootcamp: 4 keys read by name at 7 runtime sites); warn where the registry is aspirational (Sprint F+ will wire runtime bindings). Added `_LIST_BLOCKS` dispatch tuple + shared `_validate_known_ref_list` helper + table-driven `_validate_bootcamp_overrides` to keep `strategy_spec.py` under the 400-line A+B+C+D+E combined cap (298 → 393 lines). **Schema-only, no runtime consumption.** 25 new tests in `tests/platform/specs/test_schema_final_blocks.py` (one valid + one rejection per block + combined + backward-compat on lazy_prices_v1 + post_audit_ruleset_v1). Sprint prompt's registry-location claim for attribution (`src/ranking/attribution.py`) was incorrect — canonical location is `src/attribution/logger.py` (documented in Pass 2 research). Next: Sprint F (ranker port — runtime consumes spec). |
| #530 Sprint C.1 — schema refinement: scoring shape gaps (#569, #567, #568) | feat/schema-c1-ranker-gaps | **Slot 6-a of 9 in #530 chain — chain count revised 8→9; F/G/H shift to slots 7/8/9.** Sprint F Pass 1 surfaced 9 schema shape gaps blocking byte-identity port of the ranker; C.1 refines C/D/E to close them before F resumes. **9 items:** (1) categorical bands `{metric, category, score}` alongside numeric ranges, covers `trend_state`/`relative_strength_state`; (2) compound AND bands via `conditions: [{metric, operator, threshold}, ...]`, operator enum `{>, >=, <, <=, ==, !=}`, covers `iv_rank > 75 AND pc_vol > 1.2`; (3) weighted blend groups via `weight` + `blend_group` fields, covers 0.6/0.4 market-vs-sector RS blend; (4) new `ranking.adjustments` block (same band grammar + `clamp: [lo, hi]`), covers `_regime_adjustment` in ranker.py:72-102; (5) new `ranking.derived_metrics` block with `subtract`/`weighted_sum` ops + DAG cycle check, covers `_compute_sector_rs` in ranker.py:105-147; (6) #567 — `packet_worthy` → `min_score` hard-rename (field was int threshold but validator asserted bool); (7) #568 — `KNOWN_POST_SCAN_HELPERS` contents aligned to runtime `{traffic_light, event_risk}` + `strict=True` flip; (8) `KNOWN_SCORING_METRICS` 10-metric seed registry from `_score_ticker` + `_regime_adjustment`, effective set unioned with derived-metric names from Item 5; (9) event-risk casing docstring (Option 9A — no runtime edits, convention codified in comment). **Added** `KNOWN_REGIME_LABELS` 5-label set (from compute_market_regime, intentionally separate from KNOWN_REGIME_KEYS 7-label threshold-dispatch set). **Structure:** ranking validators extracted to `src/platform/_strategy_spec_ranking.py` (341 LOC) to keep `strategy_spec.py` focused; main module re-exports constants for public API stability. `strategy_spec.py`: 393 → 388 lines (under 650 guardrail). **Schema-only, no runtime consumption.** 28 new tests in `tests/platform/specs/test_schema_c1_refinements.py`; Sprint D tests updated for `min_score`, Sprint E tests updated for new post_scan.chain names + strict rejection, one Sprint C test fixture updated to use real metric names (KNOWN_SCORING_METRICS now validates). **Known Sprint F divergence (per operator 2026-04-20):** sector_rs None-fallback in `_score_ticker:182-187` is NOT expressible in pure weighted-blend schema — Sprint F will STOP on fuzz failure and file issue if it matters. **Follow-up candidates for Sprint F/C.2:** symmetric categorical-value validation for `trend_state`/`relative_strength_state`/`market_breadth_label` (~10 LOC each). Unblocks Sprint F pending #570 (2024 OHLCV) resolution. |
| **Trading-Readiness Audit** (2026-04-27, 26 commits) | feat/bootcamp-archive-friday | **Pre-Mon-deploy harsh audit. Outcome: Stage-1 honest baseline signed (`d651160`), 3-stage roadmap replaces SD#41 REVISED's raw-Sharpe gate.** Track 1 (Mon-blocking, 8/8): canonical Sharpe migration (T1.03), pre-#651 quarantine + propagation (T1.01/T1.05), `effective_position_cap` reconciliation (T1.04), configurable stop ATR (T1.06), preflight gate (T1.07), instrumentation filter + MinTRL (T1.08), Stage-1 recompute writer (T1.02). Track 2 Cohort 1 (housekeeping, 6/7 — T2.18 deferred, scope mismatch): PBO writer (T2.06), FRED rf-rate + F-16 pin (T2.10), pandas_market_calendars NYSE (T2.11), dead classifier classes dropped (T2.13), lazy-prices spec shelved (T2.15), fail-closed governor + Alpaca probe (T2.17). Track 2 Cohort 2 (toolkit, 5/5 — all shelf): CPCV (T2.01), block bootstrap (T2.02), MC permutation (T2.03), PSR/DSR/MinTRL + ≥4-of-5 promotion gate (T2.04), White RC (T2.05). Track 2 Cohort 3-Half-A (cores, 5/5 — all shelf): cost calibration (T2.07), PIT universe (T2.09), risk-parity allocator (T2.12a), pullback logistic features (T2.14a), factor-alpha core (T2.16a). Track 3 (1/2): devil's-advocate doc (T3.01); T3.02 deferred until real Stage-2/3 results exist. **Test floor: 3,038 → 3,380 (+342 net, 0 failures).** Stage-1 numbers from archive (n=35 instrumented): rf-adjusted excess Sharpe 6.14 (literal verdict GREEN; SPY-relative non-significant at p=0.43, IID bootstrap acknowledged optimistic). Selection-bias diagnostic confirmed REVERSE bias (14 dropped trades had +2.55% higher mean and 84.6% win rate vs 69%) — headline conservatively understated. New module families: `src/methods/`, `src/allocation/`, `src/cost_model/`, `src/universe/pit.py`. New dep: `pandas_market_calendars>=4.0,<6.0`. Cohort 3-Half-B (5 wiring tasks: T2.08/T2.12b/T2.14b/T2.14c/T2.16b) deferred to next session. Audit artifacts: `docs/audits/2026-04-27-trading-readiness/SHIPPED.md` + `audits/2026-04-27/{stage1_baseline_memo.md,devils_advocate_stage1.md}`. Methodology reference: `docs/methodology-toolkit.md`. |
| **Track 1.5 — Instrumentation Gap Closure** (2026-04-25 PM, 14 rounds, 16+ commits) | feature/track-1.5-instrumentation-gaps | **Post-audit PM-autonomous dispatch. All Critical + Important findings from both audit passes cleared. ~250 new tests. SD#46 fix-before-trade principle adopted. Mon $100 deploy deferred.** B1-B9 instrumentation deliverables: `signal_exit_price` + `exit_slippage_bps` at close (B1); `broker_exceptions` schema + 4 silent-swallow upgrades + 15 structured-logging sites + bounded retry/qty-mismatch detection (B2.A/B/C); `exit_reason` canonical taxonomy + nightly reconciliation (B3); `key_risk_assessment` + `expected_holding_period_days` at open (B4+B8); `instrumentation_version` INTEGER sentinel + `filter_to_version` helper + `docs/instrumentation_versions.md` (B5); end-to-end integration test (B6); `llm_timeout_days` in ledgers (B9). Round 8 dashboard fixes: 5-KPI hero strip with rf-adjusted excess Sharpe / SPY-relative / win rate / Stage traffic light / promotion-gate vote count (Round 8.B — R1 resolved); broker_exceptions panel (Round 8.C — G1 resolved); win-rate silent fallback removed (Round 8.D — R2); P&L source labels (Round 8.D — R3); preflight UI echo (Round 8.D — S4); 5 Critical backend route fixes (Round 8.A — C1-C5); 3 deferred audit cosmetics (Round 8.F). Integration test for full pipeline (Round 9a). Round 9b: this docs commit. **Hard truths documented:** Sharpe 6.14 likely regime-tailwind; SPY-relative non-significant (p=0.43); dashboard hero was showing wrong formula since T1.03 landed; MinTRL for Sharpe > 0.5 is 80-150 trades (we have 35). **Sprint queue after Track 1.5:** (1) `sections_json` widening, (2) System Index audit, (3) Council impact, (4) Cohort 3 strategy redesign + new baseline. Full decisions: `docs/audits/2026-04-27-trading-readiness/track-1.5-DECISIONS.md`. |
...
42. SD#42 — Friday bootcamp archive (scripts/archive_bootcamp_2026_04_24.py). VACUUM INTO archive + schema-only fresh-DB anchor. Archive is the immutable Stage-1 audit baseline; never modified by subsequent sprints.

43. SD#43 — 3-stage roadmap (audit-spec §3.1). Stage 1 = honest signed baseline (DONE); Stage 2 = excess Sharpe ≥ 0.5 at p < 0.05 over 150 OOS trades using block bootstrap (T2.02) + ≥4-of-5 promotion gate (T2.04); Stage 3 = > 1.0 at p < 0.05 over 300 OOS. SPY-relative non-significance (p=0.43 at Stage 1) is the diagnostic gate — Stage-2 evaluation will not authorize ramp until SPY-relative p drops below 0.10.

44. SD#44 — 5-KPI strip as the canonical dashboard hero (2026-04-25, Track 1.5). Operator-approved candidates: rf-adjusted excess Sharpe (T1.03 canonical), SPY-relative Sharpe + p-value + 95% CI lower, win rate (quarantine-filtered), Stage-1/2 traffic light per §3.1 Decision Matrix, promotion-gate vote count. Color rules documented in `track-1.5-DECISIONS.md` Decision 4. Replaces the three incompatible Sharpe formulas that coexisted across Dashboard hero / CTOReport / TradeHistory rolling chart.

45. SD#45 — `instrumentation_version` INTEGER sentinel on `shadow_trades` (Track 1.5 / B5). v0=quarantined/pre-#651, v1=pre-April-9 (no conviction), v2=April-9-to-Track-1.5 (conviction integer only), v3=post-Track-1.5 (B1+B3+B4 fully populated). Analytics rule: any performance figure MUST filter to `instrumentation_version >= 3`. DEFAULT 3 so new rows are auto-stamped. See `docs/instrumentation_versions.md`.

46. **SD#46 — Fix-everything-technically-before-trading principle (2026-04-25).** Supersedes Mon $100 ramp from SD#41 REVISED until Cohort 3 redesign produces a strategy with positive expected alpha. Rationale: Stage-1 SPY-relative p=0.4326 means we cannot reject "returns are indistinguishable from randomly long stocks in a bullish window." Deploying $100 to "find out cheap" was defensible pre-audit; post-audit the calculus is: (a) operator's gut already said "I don't feel like the strategy has alpha," (b) fix-now-before-trade principle means trading is gated on technical health AND believing in the strategy, (c) spending live-data slots on a strategy we expect to retire is making things worse, not better. Next deploy decision after Cohort 3 redesign (T2.14b/T2.14c/T2.16b) + new Stage-1 baseline. Mon AM preflight still runs as system-health check. Memory: `feedback_fix_before_trade.md`.
...
|---|---|---|---|
| Phase 1 -> 2 | **REDEFINED AGAIN (2026-04-27 audit, supersedes SD#41 REVISED):** Stage 1 honest baseline signed (DONE, commit `d651160`); Stage 2 = excess Sharpe ≥ 0.5 at p < 0.05 over 150 OOS trades using BLOCK bootstrap (T2.02) + ≥4-of-5 promotion gate (T2.04); Stage 3 = > 1.0 at p < 0.05 over 300 OOS via same gate. SPY-relative significance is the diagnostic gate — Stage 1 was non-significant (p=0.43), so Stage-2 evaluation will not authorize ramp until SPY-relative p drops below 0.10. Raw-Sharpe gate from SD#41 REVISED retired. | Stage 1 signed 2026-04-25 (n=35 instrumented from bootcamp archive, rf-adjusted excess Sharpe 6.14, literal verdict GREEN with caveats). Stage-2 OOS counter starts on first live trade post-Mon-2026-04-27 deploy. | Stage 1 ✅ DONE; Stage 2 begins 2026-04-27 if preflight green |
...
> **Methodology Toolkit (2026-04-27 audit):** [docs/methodology-toolkit.md](docs/methodology-toolkit.md) — when to use CPCV / block bootstrap / MC permutation / PSR-DSR-MinTRL / White RC / promotion gate. Toolkit is "shelf" — implemented in `src/methods/`, not yet wired into runtime promotion. Drawn from manually when evaluating a strategy.
...
| -- | **Walk-forward framework — spec + plan (Sprint S1-CC Batch B)** | QUEUED — docs-only sprint (B1/B2/B3) landing the walk-forward spec + plan after Stage 1 corpus closeout. v2 model training is gated on walk-forward **implementation** (separate later sprint), not on this docs batch. |
...
- ~~Rename GitHub repo `halcyon-lab` -> `arcis`~~ ✅ DONE 2026-04-25 (operator + `git remote set-url`)
