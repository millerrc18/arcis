...
export const api = {
  ...
  getOpenTrades: (desk) => fetchApi(`/shadow/open${desk ? `?desk=${encodeURIComponent(desk)}` : ''}`),
  getClosedTrades: (days = 30, desk) => fetchApi(`/shadow/closed?days=${days}${desk ? `&desk=${encodeURIComponent(desk)}` : ''}`),
  getAccount: (desk) => fetchApi(`/shadow/account${desk ? `?desk=${encodeURIComponent(desk)}` : ''}`),
  getMetrics: (days = 30, desk) => fetchApi(`/shadow/metrics?days=${days}${desk ? `&desk=${encodeURIComponent(desk)}` : ''}`),
  getSharpeAttribution: (desk) => fetchApi(`/shadow/sharpe-attribution${desk ? `?desk=${encodeURIComponent(desk)}` : ''}`),
