      ...
      <tr
        ...
        style={{
          background: isCritical
            ? LEVEL_COLORS.CRITICAL.bg
            : rowIndex % 2 === 0
              ? "transparent"
              : "var(--arcis-bg-elevated)",
          borderBottom: isCritical
            ? `1px solid ${LEVEL_COLORS.CRITICAL.border}`
            : undefined,
        }}>
        ...
        <td className="py-1.5 px-3">
          <StatusBadge
            text={log.log_level}
            variant={LEVEL_VARIANTS[log.log_level] || "neutral"}
          />
        </td>
        <td
          className="py-1.5 px-3 hidden md:table-cell"
          style={{
        ...
        <td
          ...
          style={{
            color:
              LEVEL_COLORS[log.log_level]?.color || "var(--arcis-text-primary)",
          }}>
          {log.message}
        </td>
      </tr>
...
export default function Logs() {
  ...
  const submitCmd = useMutation({
    mutationFn: (cmd) =>
      api.submitCommand({ command_type: cmd.type, command_name: cmd.name }),
  });
  ...
  async function handleClearStale() {
    ...
    try {
      if (api.clearStaleCommands) {
        await api.clearStaleCommands();
      } else {
        // Local UI fallback: update the cached `commands-recent` entry so the
        // dashboard reflects a cleared/expired state even when no server-side
        // sweep endpoint is available.
        queryClient.setQueryData(["commands-recent"], (old) => {
          if (!old || !old.commands) return old;
          const updated = {
            ...old,
            commands: old.commands.map((cmd) =>
              isStaleCommand(cmd) ? { ...cmd, status: "expired" } : cmd,
            ),
          };
          return updated;
        });
      ...
      <div
        ...
        {commands.length === 0 ? (
          <div
            ...
            style={{ color: "var(--arcis-text-muted)" }}>
            No commands recorded yet \u2014 use "Run Command" above or the
            dashboard to submit commands
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr
                  ...
                  <th
                    className="text-left py-1 pr-3 text-xs uppercase"
                    style={{ fontSize: "10px" }}>
                    Time
                  </th>
                  <th
                  ...
                  <th
                    className="text-left py-1 pr-3 text-xs uppercase hidden md:table-cell"
                    style={{ fontSize: "10px" }}>
                    Type
                  </th>
                  <th
                  ...
                  <th
                    className="text-right py-1 text-xs uppercase hidden md:table-cell"
                    style={{ fontSize: "10px" }}>
                    Duration
                  </th>
                </tr>
              ...
              <tbody>
                {commands.map((cmd, i) => (
                  <tr
                    ...
                    <td
                      className="py-1.5 pr-3 hidden md:table-cell"
                    ...
                    <td className="py-1.5 pr-3">
                      <StatusBadge
                        ...
                        variant={
                          (cmd.result_status || cmd.status) === "success"
                            ? "success"
                            : (cmd.result_status || cmd.status) === "pending"
                              ? "warning"
                              : (cmd.result_status || cmd.status) === "claimed"
                                ? "warning"
                                : (cmd.result_status || cmd.status) === "error"
                                  ? "error"
                                  : "neutral"
                        }
                      />
            ...
            <button
              ...
              style={{
                ...
                background:
                  levelFilter === level
                    ? "var(--arcis-accent)"
                    : "var(--arcis-bg-surface)",
                color:
                  levelFilter === level
                    ? "white"
                    : "var(--arcis-text-secondary)",
                border:
                  "1px solid " +
                  (levelFilter === level
                    ? "var(--arcis-accent)"
        ...
        <div className="overflow-x-auto max-h-[600px] overflow-y-auto">
          <table className="w-full text-sm">
            <thead
              ...
              style={{ background: "var(--arcis-bg-surface)" }}>
              <tr
                style={{
                  color: "var(--arcis-text-secondary)",
                ...
                <th
                  className="text-left py-2 px-3 text-xs uppercase"
                  style={{ fontSize: "10px" }}>
                  Time
                </th>
                <th
                ...
                <th
                  className="text-left py-2 px-3 text-xs uppercase hidden md:table-cell"
                  style={{ fontSize: "10px" }}>
                  Source
                </th>
                <th
              ...
              {logs.length === 0 ? (
                <tr>
                  <td
                    ...
                    style={{ color: "var(--arcis-text-muted)" }}>
                    System logs will appear here once the watch loop starts
                    recording
                  </td>
                </tr>
              ) : (
