...
const initialNodes = [
  ...
  // Row 2: Processing
  { id: 'features', position: { x: 150, y: 120 }, data: { label: 'Feature Engine\nTechnicals + Regime + Sector' }, style: nodeStyle(SLATE, TEAL) },
  { id: 'enrichment', position: { x: 450, y: 120 }, data: { label: 'Data Enrichment\n7 Sources + PEAD' }, style: nodeStyle(SLATE, TEAL) },
  { id: 'collectors', position: { x: 750, y: 120 }, data: { label: '12 Overnight\nCollectors' }, style: nodeStyle(SLATE, TEAL) },

  // Row 3: Decision & Risk
  { id: 'scoring', position: { x: 100, y: 250 }, data: { label: 'Ranking\nScore 0-100' }, style: nodeStyle(SLATE, AMBER) },
  { id: 'traffic_light', position: { x: 300, y: 250 }, data: { label: 'Traffic Light\nRegime Gate' }, style: nodeStyle(SLATE, AMBER) },
  { id: 'risk', position: { x: 500, y: 250 }, data: { label: 'Risk Governor\n8 Checks + Kill Switch' }, style: nodeStyle(SLATE, RED) },
  { id: 'llm', position: { x: 750, y: 250 }, data: { label: 'halcyon-v1\nPacket Writer' }, style: nodeStyle(SLATE, PURPLE) },

  // Row 4: Execution
  { id: 'shadow', position: { x: 200, y: 380 }, data: { label: 'Shadow Execution\nAlpaca Brackets' }, style: nodeStyle(SLATE, GREEN) },
  { id: 'live', position: { x: 450, y: 380 }, data: { label: 'Live Execution\nAlpaca Paper/Live' }, style: nodeStyle(SLATE, GREEN) },
  { id: 'reconcile', position: { x: 700, y: 380 }, data: { label: 'Reconciliation\nEvery 15 min' }, style: nodeStyle(SLATE, GREEN) },

  // Row 5: Training Flywheel
  { id: 'blinding', position: { x: 50, y: 510 }, data: { label: 'Self-Blinding\nGeneration' }, style: nodeStyle(SLATE, PURPLE) },
  { id: 'quality', position: { x: 250, y: 510 }, data: { label: 'Quality Scoring\nLLM-as-Judge' }, style: nodeStyle(SLATE, PURPLE) },
  { id: 'leakage', position: { x: 450, y: 510 }, data: { label: 'Leakage\nDetection' }, style: nodeStyle(SLATE, PURPLE) },
  { id: 'curriculum', position: { x: 650, y: 510 }, data: { label: 'Curriculum SFT\n3-Stage Training' }, style: nodeStyle(SLATE, PURPLE) },
  { id: 'eval', position: { x: 850, y: 510 }, data: { label: 'A/B Eval\n+ Holdout' }, style: nodeStyle(SLATE, PURPLE) },

  // Row 6: Infrastructure
  { id: 'scheduler', position: { x: 100, y: 640 }, data: { label: 'Watch Loop\n24/7 Scheduler' }, style: nodeStyle(SLATE, '#64748B') },
  { id: 'dashboard', position: { x: 350, y: 640 }, data: { label: 'Arcis Dashboard\n16 Pages' }, style: nodeStyle(SLATE, '#64748B') },
  { id: 'telegram', position: { x: 600, y: 640 }, data: { label: 'Telegram\nNotifications' }, style: nodeStyle(SLATE, '#64748B') },
  { id: 'render', position: { x: 830, y: 640 }, data: { label: 'Render\nCloud Deploy' }, style: nodeStyle(SLATE, '#64748B') },
]

const edgeDefaults = { type: 'smoothstep', animated: true, style: { stroke: '#475569', strokeWidth: 1.5 } }
