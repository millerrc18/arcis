  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg p-4" style={{ background: 'var(--arcis-bg-surface)', border: '1px solid var(--arcis-border)' }}>
        <h3 className="text-sm uppercase tracking-wide mb-4" style={{ color: 'var(--arcis-text-secondary)' }}>Dataset Breakdown</h3>
        <div className="flex gap-1 h-4 rounded-full overflow-hidden" style={{ background: 'var(--arcis-border)' }}>
          {status?.dataset_total > 0 && (
            <>
              ...
              <div style={{ width: `${(status.dataset_wins / status.dataset_total) * 100}%`, background: 'var(--arcis-success)' }} />
              <div style={{ width: `${(status.dataset_losses / status.dataset_total) * 100}%`, background: 'var(--arcis-danger)' }} />
            </>
          )}
        </div>
