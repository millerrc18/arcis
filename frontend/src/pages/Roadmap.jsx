    ...
    { id: 'p1', name: 'Phase 1 — Bootcamp (Diagnostic)', status: 'active', capital: '$100K paper + $100 live', cost: '$64/mo', timeline: 'Apr 2026 – (extended per SD#41 REVISED)',
      ...
      subphases: [
        ...
        { label: 'Weeks 8–12: Risk & NLP', items: [
          ...
          { l: 'Earnings 7-day exclusion zone (SD#33)', s: 'pending', c: 'risk', d: 'Block entries within 7 calendar days of earnings. Force-exit open positions 2 days before. 2-business-day cooldown after. Median mega-cap gaps breach 2x ATR stops. PEAD dead for large caps.', r: 'Deep Research: earnings-event-handling-pullback-strategy.md — Martineau 2022, Subrahmanyam 2025' },
          { l: '3-regime classifier v2 (SD#35)', s: 'pending', c: 'strategy', d: 'Replace 7-label classifier (75% unknown) with priority-ordered bull/cautious/bear. Add VIX/VIX3M ratio. 5-day debounce. Hysteresis. Kill range and unknown.', r: 'Deep Research: regime-classifier-fix-3-regimes.md — Hamilton 1989, Statistical Jump Models' },
          { l: 'Monthly retraining cadence (SD#34)', s: 'pending', c: 'ai', d: 'Full reset from Qwen3-8B base monthly (not weekly). 5-10 weekly examples below noise floor. Canary perplexity trigger. FP16 master copy mandatory. Disable auto-retrain.', r: 'Deep Research: optimal-retraining-cadence-lora.md — Zhang ICLR 2024, Shuttleworth NeurIPS 2025' },
          { l: 'TCA logging (SD#38)', s: 'pending', c: 'ops', d: 'Log NBBO at signal time, fill price, fill timestamp, exit type for every trade. Market impact zero at $5-25K. Broker selection = 39 bps spread between best/worst. Build now, analyze at 200+ trades.', r: 'Deep Research: transaction-cost-analysis-sp100.md — Schwarz 2025, Frazzini 2018' },
        ...
        { label: 'Strategy Research Platform (v0.24.0 — April 2026 pivot)', items: [
          { l: 'Backtest harness (deterministic, strategy-agnostic)', s: 'pending', c: 'validation', d: 'Event-driven historical replay. Reuses simulate_mechanical_outcome (yesterday\'s D2 forensic audit) + spy_benchmark.py (D1) + fetch_cached_ohlcv (parquet cache). Non-negotiable: hand-computed validation test (scheduled + event-driven paths) must pass before any strategy evaluation is trusted.', r: 'Spec: docs/sprints/sprint-research-platform.md — Task 4' },
          { l: 'Strategy specification format (YAML + Python plugin)', s: 'pending', c: 'strategy', d: 'Declarative YAML for simple strategies, Python plugin ABC for complex. Every strategy publishes expected factor profile before going live (catches implementation bugs P&L alone will miss).', r: 'Platform spec: Tasks 1, 2' },
          { l: 'Deflated Sharpe Ratio promotion gate (Bailey-LdP 2014)', s: 'pending', c: 'validation', d: 'REPLACES naive excess_sharpe ≥ 0.5 gate. Honest annualized-Sharpe hurdle for 30 serial trials is ~1.0-1.3, not 0.5. Unit test reproduces paper p.9 example (DSR=0.9004, SR*_0_ann=0.5429) to 4 decimals. Without DSR, 50% of passing strategies would be noise.', r: 'Deep research: backtest-rigor-retrofit-plan.pdf — Bailey & Lopez de Prado 2014 JPM 40(5)' },
          { l: 'CSCV / PBO + rolling walk-forward', s: 'pending', c: 'validation', d: 'Combinatorially Symmetric Cross-Validation at S=16 partitions. Pardo 2008 walk-forward with OOS efficiency gate (OOS_SR / IS_SR ≥ 0.3). Reject strategies with PBO > 0.5 when parameter sweeps involved.', r: 'Deep research: backtest-rigor-retrofit-plan.pdf — Bailey-Borwein-LdP-Zhu 2014' },
          { l: 'Survivorship bias haircut', s: 'pending', c: 'validation', d: 'Apply -75 bps/yr for short-hold, -200 bps/yr for momentum, -100 bps/yr baseline to all annualized returns before gate comparison. S&P 100 turnover 3-6 deletions/year; GE/Monsanto/Yahoo/Kraft are biggest missing losers. Full PIT universe build is a separate 6-12h project deferred to v0.24.1.', r: 'Deep research: Brown-Goetzmann-Ibbotson-Ross 1992, Harris S&P 100 measurements' },
          { l: 'Task 0 EDGAR fetch pipeline repair', s: 'pending', c: 'data', d: 'Current state: 0/3,362 edgar_filings have populated full_text or sections_json. _fetch_filing_text has been silently returning None since collection began. Upstream blocker for Lazy Prices and any filing-dependent strategy. 3-6h diagnostic + backfill at SEC rate limits.', r: 'CC skeptical review: research-desk-sprint-review.md' },
          { l: 'Lazy Prices strategy (first tenant)', s: 'pending', c: 'strategy', d: 'Filing-anchored earnings drift on 14-28 day holds (Cohen-Malloy-Nguyen JF 2020, ~188 bps/mo alpha t=2.76). YoY cosine similarity on 10-K/10-Q Risk Factors + MD&A. Long-non-changers after positive surprise. Uncorrelated with swing (event-driven fundamental drift, not price mean-reversion).', r: 'Deep research: research-desk-design-report.md' },
          { l: 'Per-desk Alpaca client factory (12 call sites)', s: 'pending', c: 'ops', d: 'Modify _get_trading_client and _get_data_client to accept desk parameter. Thread desk through 4 external callers (executor.py, reconcile.py which is ACTIVELY called, bracket_monitor.py, shadow_service.py). verify_accounts_distinct() prevents both desks pointing at same account.', r: 'Platform spec: Task 7; CC skeptical review call-site audit' },
          { l: 'Shadow-trading harness (research account)', s: 'pending', c: 'strategy', d: 'Runs validated strategy specs on Alpaca Paper Account #2. Logs to shadow_trades with desk=research_<strategy_id>. Halt closes open positions via research Alpaca client.', r: 'Platform spec: Task 7' },
          { l: 'Promotion pipeline (proposed→backtested→shadow→production)', s: 'pending', c: 'validation', d: 'Strategy registry + promotion events log + trials_registry for global N_eff counter. Gates: DSR ≥ 0.95, PBO ≤ 0.50, OOS efficiency ≥ 0.30. Manual promotions require ≥40-char justification note (decision fatigue mitigation).', r: 'Platform spec: Task 10' },
          { l: 'Correlation monitoring stack (Longin-Solnik + Carhart+QMJ)', s: 'pending', c: 'risk', d: 'Three-layer stack: Spearman (primary, robust) + Pearson (fat-tail divergence flag) + negative exceedance correlation (tail dependence). Carhart 4 + QMJ factor decomposition with Newey-West HAC standard errors. PELT change detection on factor betas. Flags style drift before P&L damage.', r: 'Deep research: correlation-risk-monitoring-blueprint.pdf — Longin-Solnik 2001, Ang-Chen 2002, Carhart 1997, AQR QMJ 2019' },
          { l: 'Hard exposure limits (Millennium/Citadel architecture)', s: 'pending', c: 'risk', d: '6% single-name cap, 25% sector cap, 1.5x gross leverage, 8% book drawdown circuit breaker. Enforced pre-trade in code. Never overridden during drawdown. Aggregate caps across strategies (ticker is unit of risk, not strategy).', r: 'Deep research: correlation-risk-monitoring-blueprint.pdf — pod-shop exposure architecture' },
          { l: 'Defensive dashboard desk filtering', s: 'pending', c: 'ops', d: 'Dashboard.jsx + all /api/shadow/* endpoints accept ?desk= param. Default swing-only for backward compat. MUST ship before any research strategy goes shadow_trading active — otherwise swing metrics silently lie.', r: 'Platform spec: Task 12c — hard sequencing gate' },
          { l: 'Strategy Research dashboard page', s: 'pending', c: 'ops', d: '/research-platform page. Strategy registry table, backtest results grid, promotion events log, action buttons for trigger/approve/demote, home-screen platform status widget, Telegram event notifications. Full UX for operating the platform from the dashboard.', r: 'Platform spec: Task 12a/b/d/e' },
        ]},
      ],
    },
